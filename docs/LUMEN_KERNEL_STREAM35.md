...
* **Structure**: `language/structure/structural.rs` folds case, drops the prologue and comments (several block-comment pairs allowed), ends every line with a NEWLINE token outside brackets, and synthesises INDENT and DEDENT tokens for an indentation language, leaves the paired delimiters in place for a brace language, or leaves the closing word in place for a keyword-block language (`then` ... `end`), all as token-stream transformations; for a postfix language it also keeps the spaces between words, which the parser skips as separators
* **Postfix**: `language/postfix.rs` hosts a reverse Polish language (RPLumen) with one statement handler that matches every token: the stack is a thread-local vector, each word is a statement node (a literal pushes, an operator pops and pushes, a control word pops its condition and runs a body parsed by the structural pass, a quoted name is taken by the word after it), a program value `« ... »` is registered as a function of no parameters and runs through the same call path as a Lumen function; the literal handlers and the arithmetic and comparison operations are shared, nothing else is registered
...
* **The floor**: `arithmetic.rs` computes `+ - * // /` and derives `%`, `**` and unary minus from them; `comparison.rs` computes `==` and `<` and derives the other four; `control_for.rs` is a counted loop over the range's two expressions, so no range value exists; `int`, `frac` and every renderer are library code over `num`, `den` and `precision`

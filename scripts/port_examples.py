...
langs/lib_lumen/*.lm, parses them, and writes each example in the spelling of
...
The library itself is ported the same way: every function of
langs/lib_lumen/ the language can spell is written into langs/lib_<language>/,
file for file,
...
def write_mirror(lang, d, files, reasons):
    """langs/lib_<language>/: one file per library file with the functions the
    language can spell, and a prelude.rs the host embeds."""
    ext = d["extensions"][0]
    out = ROOT / "langs" / f"lib_{lang}"
    ...
    for name in LIB_FILES:
        ...
        lines = []
        if comment:
            lines.append(f"{comment} {LIBRARY_HEADER.format(source='langs/lib_lumen/' + name)}")
            lines.append("")

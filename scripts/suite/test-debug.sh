...
show_help() {
    ...
    echo "  <kernel>                stream35, microcode11, microcode4, microcode7, stack5 or stack8"
    echo ""
    ...
    case "$1" in
        ...
        --kernel)
            ...
            case "$KERNEL_FILTER" in
                stream35|microcode11|microcode4|microcode7|stack5|stack8) shift 2 ;;
...
if ! true 2>/dev/null; then echo -e "${RED}Build failed!${NC}"; exit 1; fi
...
for lang in "${LANGUAGES[@]}"; do
    for kernel in stream35 microcode11 microcode4 microcode7 stack5 stack8; do

...
| `fixwrite.py <root> <piece/n>...` | Writes a moved `.out` record from a successful run's stdout, or an `.err` record from the current binary's first stderr line. Use only once both kernels print the same new line and no `.` was lost on either. |

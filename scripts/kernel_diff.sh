...
# implementations agree on what they print. The microcode10 kernel is the
...
for f in $(find examples -type f \( -name "*.lm" -o -name "*.rpl" -o -name "*.py" -o -name "*.rs" -o -name "*.php" -o -name "*.rb" -o -name "*.pas" -o -name "*.c" -o -name "*.js" -o -name "*.swift" \) | sort); do
    ...
    reference=$($B --kernel microcode10 $flag "$f" 2>&1)
    agree=1
    for kernel in stream35 stack26 microcode11 microcode4 stack5 stack8 microcode7; do
        # shellcheck disable=SC2086
        other=$($B --kernel $kernel $flag "$f" 2>&1)
        if [ "$other" != "$reference" ]; then
            agree=0
            echo "== $f ($kernel vs microcode10)"

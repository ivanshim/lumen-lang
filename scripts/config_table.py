#!/usr/bin/env python3
"""Render the language definitions in configs/ as one comparison table.

Reads every configs/*.json, checks that all files carry the same labels in
the same order, and rewrites the table between the markers in
configs/README.md. Exit status is non-zero if the files disagree on their
...
def main() -> int:
    files = sorted(CONFIGS.glob("*.json"), key=lambda p: (FIRST.index(p.stem) if p.stem in FIRST else len(FIRST), p.stem))
    ...
    print(f"config table: {len(names)} languages, {len(reference)} labels")
    return 0

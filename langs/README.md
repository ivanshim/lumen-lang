...
- `ext.builtin.range.value`: a switch making the range builtin a value
  ...
  and walks need no array made beforehand. A slice keeps another counted
  walk where `ext.builtin.slice` is spelled.
...
- `ext.stmt.del`: takes names, indexed places and properties away, as
  ...
  whose target cannot yet be taken away. Slice targets remove each place
  named by their step, closing the gaps left behind.
...
- `ext.builtin.itemgetter` keeps one or more keys in a callable value.
  When called, it reads those keys from the value handed over; several
  answers form a tuple. `builtin.get` may spell a direct keyed read.
...
- `ext.op.index.slice`: the signs between the bounds of a span inside
  ...
  separated by the call separator form a tuple key. User index methods
  take the key whole, including ellipses. Builtin rows refuse such a
  key, and a compound slice write still stops with `.unsupported`. Index brackets may follow a piped member or method
...
| `builtin.get` | - | `get` | `operator.getitem` | - | - | - | - | - | - | - |
| `builtin.put` | - | `put` | - | - | - | - | - | - | - | - |
...
| `ext.builtin.itemgetter` | - | - | `operator.itemgetter` | - | - | - | - | - | - | - |
...
| `ext.op.index.slice.equal` | - | - | `__eq__` | - | - | - | - | - | - | - |

...
the token stream by its block style and markers. The four definitions are
embedded at build time and picked by file extension or `--lang <name>`;
`--lang file.json` reads a definition from disk instead. Every example runs
on both kernels, and `scripts/kernel_diff.sh` requires them to print the
same thing.

...
- `ext.lexical.string.prefix.raw`, `.bytes`, `.plain` and `.format`:
  ...
  even one shielding a quote. Byte text is read whole; a run without a byte value
  stops in the words of `ext.lexical.string.bytes.unavailable`. Doubled braces in formatted text stand for single braces;
...
- `ext.lexical.string.bytes.unavailable`: the words said upon reaching
  ...
  The literal is read even in a body that is never called.

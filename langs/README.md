...
- `ext.syntax.call.bind_names`: a switch; call labels bind arguments by
  name, defaults fill the places left empty, and a name given twice or
  not wanted is refused. Defaults are worked out where the routine is
  written and taken away with it. Without this switch labels retain their

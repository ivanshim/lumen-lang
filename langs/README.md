...
23. The renderers are not kernel builtins. `precision` reads the
    significant digits a real carries, and Lumen's library renders every
    value with `kind`, `num`, `den`, `int`, `frac`, `precision`, `len` and
    `char_at` (`lib_lumen/render.lm`). `to_string`, `to_int` and `to_real`

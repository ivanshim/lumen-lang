...
The language is picked from the file extension (`.lm`, `.rs`, `.py`) or with
`--lang lumen|rust_core|python_core`. Arguments after the file are passed to
the program.
...
language as data. Each language is a YAML document in
`kernels/microcode/schemas/` holding tables: lexemes, block rules, literal
words, operator precedence mapped onto kernel operations, statement keywords
mapped onto statement forms, the surface names of built-ins, and the names of
...
| Rust-like | `.rs` | braces, semicolons, `let` | yes | yes |
| Python-like | `.py` | indentation, word-form logic operators | yes | yes |
...
- [docs/LUMEN_KERNEL_MICROCODE.md](docs/LUMEN_KERNEL_MICROCODE.md) — microcode kernel and schema format

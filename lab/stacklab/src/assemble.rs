...
pub fn assemble(toks: &[Tok], def: &Def, table: &mut Table) -> Outcome<Rc<Program>> {
    ...
    Ok(Rc::new(Program { name: unit.name, params: Vec::new(), names: unit.names, yields: false, words: fuse(unit.words) }))
}
...
fn as_arg(w: &Word) -> Option<Arg> {
    match w {
        Word::Load(s) if !s.take => Some(Arg::Slot(s.clone())),
        Word::Lit(v) => Some(Arg::Lit(v.clone())),
        _ => None,
    }
}

/// The peephole of cycle 1: a compare-and-jump and an increment, each
/// one word where there were four. Jump targets are moved to match; a
/// group is left alone if a jump lands inside it.

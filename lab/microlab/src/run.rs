...
impl<'a> Runner<'a> {
    ...
    fn eval(&mut self, node: &Node, frame: &Rc<Frame>) -> R {
        match node {
            ...
            Node::Step { slot, by } => {
                let v = self.read(slot, frame)?;
                let r = match v {
                    Value::Int(x) => match x.checked_add(*by) {
                        Some(y) => Value::Int(y),
                        None => self.operate(Op::Add, "", &[v, Value::Int(*by)])?,
                    },
                    other => self.operate(Op::Add, "", &[other, Value::Int(*by)])?,
                };
                self.write(slot, frame, r.clone())?;
                Ok(r)
            }
            Node::Binary { op, name, a, b } => {
                ...
                match fast {
                    Some(v) => Ok(v),
                    None => Ok(self.operate(*op, name, &[av, bv])?),
                }
            }
            Node::If { test, then, otherwise } => {
                if self.eval(test, frame)?.truth() {
                    self.eval(then, frame)
                } else {
                    self.eval(otherwise, frame)
                }
            }
            Node::Loop { test, body, step, after } => {
                loop {
                    if !after && !self.eval(test, frame)?.truth() {
                        break;
                    }

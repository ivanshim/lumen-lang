# History of the work, batch by batch

This is the diary the handover used to carry: what each batch held, what was measured, and what was learnt on the way, oldest first. HANDOVER.md now carries only the current state, the working rules and the machine; read this when a question starts with "why is it like this".

## 1. Where things stood, from the first integration on
...
## 2. What was waiting on branches at the first integration
...
### 1z. Batch 17 merged as #504; batch 18 is math, f-string wording, strings, builtins, enumerate — and the move

Pull request #504 merged into main at 3366ea6 with all six jobs green.
CI's first run caught one thing the local checks had waved through:
scratch/file-iter/12 was taken for an old mismatch, but on main it had
matched. The file road had exposed a microcode7 fault: a class whose
`__next__` steps on another thing's own `__next__` ended a `for` loop
cleanly but made `list()` and `join()` report "special method returned
an invalid value", because the gathering road did not reclaim the parked
StopIteration. Fixed in 6eaee6b; its record moved.

Batch 18 folds five branches. fix/math-module: `math.fma` and the 3.14
names (fmin, fmax, cbrt, signbit, isnormal, issubnormal, sumprod), logs
of huge integers, `fsum` rounded as CPython does, an integer too large
for a real raising OverflowError instead of reading as infinity, and a
working `Decimal` with `localcontext` (test_math 28 to 51).
fix/fstring-syntax: CPython 3.14's wording for incompatible string
prefixes, unterminated strings and f-strings, and backslashes after a
line continuation (test_fstring 42 to 45; syntax-modern/23's record
moved to it). fix/str-methods: `swapcase`, `rindex`, character tests and
case mapping through the full Unicode tables on the `getattr` road too,
CPython's limits and messages for huge widths and precisions,
`format_map` through the mapping's own subscript, argument-count
wording, `str.replace(count=)` and `random.choices` (test_str 60 to 81
on stack8, 74 to 90 on microcode7). fix/builtin-misc: `map(strict=)`,
`breakpoint()` with `sys.breakpointhook` and PYTHONBREAKPOINT over a
working `os.environ` guard, `isatty()` on the standard streams, proper
TypeError and ValueError from the class tools and `__slots__`, the
singleton kinds' constructors (with str-methods, test_builtin 31 to 46).
fix/enumerate: `enumerate(iterable=, start=)`, `reversed()` through
`__getitem__` and `__len__`, `__length_hint__`, builtin docstrings, and
enumerate, reversed and range iterators round-tripping through pickle
(test_enumerate 57 to 77, test_range 14 to 17). The merges collided on
roster lines and on microcode7's `BUILTIN_LABELS` length (281); each
line keeps both sides.

The batch-18 count stands at stack8 1,545 of 2,738 (+98) and microcode7

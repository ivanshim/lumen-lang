...
# Usage: ./test.sh [--lang all|<language>] [--kernel stream|microcode|stack|microcode2|microcode3] [--omit file1 file2 ...]
#        ./test.sh <file>
...
show_help() {
    ...
    echo "  <kernel>                stream, microcode, stack, microcode2 or microcode3"
    echo ""
    ...
    case "$1" in
        ...
        --kernel)
            ...
            case "$KERNEL_FILTER" in
                stream|microcode|stack|microcode2|microcode3) shift 2 ;;
...
for lang in "${LANGUAGES[@]}"; do
    for kernel in stream microcode stack microcode2 microcode3; do
...
if [ -z "$KERNEL_FILTER" ]; then test_kernels=(stream microcode stack microcode2 microcode3); else test_kernels=("$KERNEL_FILTER"); fi

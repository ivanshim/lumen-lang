...
use crate::languages::python::registry::{ExprInfix, Precedence, Registry};
use crate::kernel::runtime::{Env, Value};
use crate::languages::python::numeric;
use crate::languages::python::values::{PythonNumber, PythonBool, as_number, as_bool};
...
impl ExprNode for ComparisonExpr {
    fn eval(&self, env: &mut Env) -> LumenResult<Value> {
        ...
        if let (Ok(left_num), Ok(right_num)) = (as_number(l.as_ref()), as_number(r.as_ref())) {
            ...
            return Ok(Box::new(PythonBool::new(result)));
        }

...
use crate::languages::rust::registry::{ExprInfix, Precedence, Registry};
use crate::kernel::runtime::{Env, Value};
use crate::languages::rust::numeric;
use crate::languages::rust::values::{RustNumber, RustBool, as_number, as_bool};
...
impl ExprNode for ComparisonExpr {
    fn eval(&self, env: &mut Env) -> LumenResult<Value> {
        ...
        if let (Ok(left_num), Ok(right_num)) = (as_number(l.as_ref()), as_number(r.as_ref())) {
            ...
            return Ok(Box::new(RustBool::new(result)));
        }

        // Use the trait method for generic equality
        match self.op.as_str() {
            "==" => {
                let result = l.eq_value(r.as_ref())?;
                Ok(Box::new(RustBool::new(result)))
            }
            "!=" => {
                let result = l.eq_value(r.as_ref())?;
                Ok(Box::new(RustBool::new(!result)))

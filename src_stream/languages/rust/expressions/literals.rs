...
use crate::languages::rust::registry::{ExprPrefix, Registry};
use crate::kernel::runtime::{Env, Value};
use crate::languages::rust::values::{RustNumber, RustBool};
...
impl ExprNode for NumberLiteral {
    fn eval(&self, _env: &mut Env) -> LumenResult<Value> {
        Ok(Box::new(RustNumber::new(self.value.clone())))
    }
}
...
impl ExprNode for BoolLiteral {
    fn eval(&self, _env: &mut Env) -> LumenResult<Value> {
        Ok(Box::new(RustBool::new(self.value)))
    }
}

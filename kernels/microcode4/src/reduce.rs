...
pub fn reduce(toks: &[Tok], spec: &Spec, seeded: &[String], assumed: HashMap<String, Arity>, strict: bool) -> Out<Reduced> {
    ...
    let body = if spec.layout == Layout::Postfix {
...
fn leaves_value(node: &Node) -> bool {
    match node {
        ...
        Node::Call(Target::Op(Op::If, _), args) => args[1..].iter().any(|a| match a {
            Node::Literal(Value::Code(p)) => leaves_value(&p.body),
            _ => false,
        }),
        _ => false,
    }
...
impl<'a> Reducer<'a> {
    ...
    fn assign(&mut self, name: &str, value: Node) -> Node {
        Node::Assign(self.write_slot(name), Box::new(value))
    }
    ...
    fn branch(&mut self, test: Node, then: Node, otherwise: Node) -> Node {
        call(Op::If, vec![test, then, otherwise])
    }
        ...
        S: FnOnce(&mut Self) -> Out<Node>,
    {
        ...
        Ok(seq(vec![Node::Assign(slot, Box::new(program)), run(Node::Load(start), Vec::new())]))
    }
    ...
    fn postfix_body(&mut self, stops: &[String]) -> Out<(Vec<Node>, Vec<Node>)> {
        let mut stmts = Vec::new();
    ...
    fn postfix_block(&mut self, stops: &[String]) -> Out<(Vec<Node>, Option<Node>)> {
    ...
    fn named(&mut self, taker: &Tok, name: &str, stmts: &mut Vec<Node>, stack: &mut Vec<Node>) -> Out<()> {
        ...
        if spec.is("stmt.for", word) {
            ...
            let node = self.counted(name, start, end, move |r| {
                let (s, left) = r.postfix_block(&closers)?;
                r.loop_body(s, left)
            })?;
            self.want_closer()?;
            stmts.push(node);
    ...
    fn word(&mut self, t: &Tok, stmts: &mut Vec<Node>, stack: &mut Vec<Node>) -> Out<()> {
        ...
        if spec.is("stmt.if", w) {
            ...
            let then = self.arm(Catch::Nothing, |r| {
                let (mut s, left) = r.postfix_block(&stops)?;
            ...
            let otherwise = if self.at_any("stmt.else") {
                self.next();
                self.arm(Catch::Nothing, |r| {
                    let (mut s, left) = r.postfix_block(&closers)?;
        ...
        if spec.is("stmt.while", w) {
            self.spill(stmts, stack);
            let intros = spec.list("block.intro").to_vec();
            let node = self.looping(
                move |r| {
                    let (s, left) = r.postfix_block(&intros)?;
                    r.want_intro()?;
                    let mut items = s;
        ...
        if let Some(k) = spec.list("stack.program.open").iter().position(|o| o == w) {
            ...
            let (mut s, left) = self.postfix_block(std::slice::from_ref(&close))?;
            self.want_lex(&close)?;
            if let Some(v) = left {
        ...
        if spec.one("syntax.array.open") == Some(w) {
            ...
            let (inner, items) = self.postfix_body(std::slice::from_ref(&close))?;
            self.want_lex(&close)?;
            if !inner.is_empty() {
                self.spill(stmts, stack);
                stmts.extend(inner);

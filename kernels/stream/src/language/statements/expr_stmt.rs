...
impl StmtHandler for ExprStmtHandler {
    fn matches(&self, parser: &Parser) -> bool {
        ...
        let statement_words = [
            "stmt.if", "stmt.elif", "stmt.else", "stmt.while", "stmt.until", "stmt.for", "stmt.break",
            "stmt.continue", "stmt.return", "stmt.function", "stmt.let", "stmt.pass", "builtin.extern",
        ];
        ...
        if let Some(ch) = lexeme.chars().next() {
            if ch.is_numeric() || d.chars("lexical.string_quotes").contains(&ch) {
                return true;
            }
        }

...
impl NumberMarks {
    pub fn from_definition() -> Self {
        ...
        NumberMarks {
            ...
            hex_prefix: d.list("lexical.number.hex_prefix").first().cloned(),
        }
    }
}
...
pub fn parse_number_rational(s: &str) -> LumenResult<(BigInt, BigInt)> {
    ...
    // A hexadecimal literal carries the prefix.
    if let Some(digits) = marks.hex_prefix.as_deref().and_then(|p| s.strip_prefix(p)) {
        let num = BigInt::parse_bytes(digits.as_bytes(), 16)
            .ok_or_else(|| format!("Failed to parse number: {}", s))?;
        return Ok((num, BigInt::from(1)));
    }

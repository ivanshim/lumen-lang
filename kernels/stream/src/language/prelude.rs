...
pub use crate::language::definition::def;
pub use crate::language::registry::{
    parse_expr_with_prec, ExprInfix, ExprPrefix, Precedence, Registry, StmtHandler,
};
pub use crate::language::{word_char, word_start};
    ...
    fn at_identifier(&self) -> bool {
        ...
        match (def().first_char("identifier.variable_prefix"), lexeme.chars().next()) {
            (Some(prefix), Some(c)) if prefix == c && lexeme.chars().count() == 1 => {
                self.peek_n(1).map_or(false, |next| next.lexeme.chars().next().map_or(false, word_start))
            }
            _ => false,
        }
    }

    /// Consume an identifier: an optional variable prefix, the token that
    /// begins the word, and every following single-character token that
    /// continues it. The prefix is part of the name.
    fn take_identifier(&mut self) -> Option<String> {

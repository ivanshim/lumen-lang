...
use crate::languages::python_core::registry::{ExprPrefix, Registry};
use crate::kernel::runtime::{Env, Value};
use crate::languages::python_core::values::{PythonCoreNumber, PythonCoreBool};
...
impl ExprNode for NumberLiteral {
    fn eval(&self, _env: &mut Env) -> LumenResult<Value> {
        Ok(Box::new(PythonCoreNumber::new(self.value.clone())))
    }
}
...
impl ExprNode for BoolLiteral {
    fn eval(&self, _env: &mut Env) -> LumenResult<Value> {
        Ok(Box::new(PythonCoreBool::new(self.value)))
    }
}

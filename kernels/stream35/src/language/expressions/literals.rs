...
use crate::language::values::{LumenNumber, LumenBool, LumenString, LumenNull, LumenReal};
use crate::language::numeric::{self, NumberMarks};
...
impl ExprPrefix for NumberLiteralPrefix {
    ...
    fn parse(&self, parser: &mut Parser, _registry: &super::super::registry::Registry) -> LumenResult<Box<dyn ExprNode>> {
        ...
        // A hexadecimal literal: the prefix's digit, its letter, then hex digits
        if let Some(prefix) = &marks.hex_prefix {
            let mut p = prefix.chars();
            let (digit, letter) = (p.next(), p.next());
            let next_is_hex = parser.peek_n(1).map_or(false, |t| t.lexeme.chars().all(|c| c.is_ascii_hexdigit()) && !t.lexeme.is_empty());
            if Some(value.as_str()) == digit.map(|c| c.to_string()).as_deref()
                && parser.peek().lexeme.chars().next() == letter
                && parser.peek().lexeme.chars().count() == 1
                && next_is_hex
            {
                value.push_str(&parser.advance().lexeme);
                while parser.peek().lexeme.chars().count() == 1
                    && parser.peek().lexeme.chars().next().map_or(false, |c| c.is_ascii_hexdigit())
                {
                    value.push_str(&parser.advance().lexeme);
                }
                return Ok(Box::new(NumberLiteral { value }));
            }
        }

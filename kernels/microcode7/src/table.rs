...
const EXT_TAGS: &str = "\
ext.stmt.type_params.close:L \
ext.stmt.type_params.open:L \
...
ext.stmt.yield:L \
ext.stmt.async.unready:L \
ext.op.await:L \
ext.stmt.async:L \
ext.stmt.with.unready:L \
ext.stmt.with.as:L \
ext.stmt.with:L \
ext.op.lambda.unready:L \
ext.op.lambda:L \

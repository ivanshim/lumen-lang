...
ext.op.instanceof:L ext.stmt.class.parent:L ext.stmt.class.self:L ext.lexical.name_lead:L ext.stmt.assert:L ext.stmt.assert.kind:L ext.stmt.catch.invalid:L ext.stmt.catch.as:L ext.stmt.catch.tuple.open:L ext.stmt.catch.tuple.close:L ext.lexical.string.long:L ext.lexical.string.adjacent:B ext.lexical.escape.continued:B ext.stmt.with:L ext.stmt.with.as:L ext.stmt.with.unready:L ext.stmt.del:L ext.stmt.del.unrun:L ext.stmt.nonlocal:L ext.stmt.nonlocal.unrun:L ext.stmt.yield:L ext.stmt.yield.from:L ext.stmt.yield.unrun:L ext.op.member.pipes:B ext.op.tuple:L ext.op.tuple.unready:L ext.lexical.string.prefix.raw:L ext.lexical.string.prefix.plain:L ext.lexical.string.prefix.bytes:L ext.lexical.string.prefix.format:L ext.lexical.string.prefix.bytes.unready:L ext.lexical.string.prefix.format.unready:L ext.lexical.string.amiss:L ext.op.identical.negated:L ext.op.identical.unsupported:L ext.op.in:L ext.op.in.negated:L ext.op.in.unsupported:L ext.op.if_else:L ext.stmt.assign.chain:B ext.lexical.escape.deferred:L ext.lexical.escape.unavailable:L ext.stmt.class.unready:L ext.stmt.catch.group:L ext.stmt.catch.group.unsupported:L ext.stmt.try.else:B ext.stmt.throw.from:L ext.stmt.throw.empty:L ext.stmt.try:L \
...
impl Table {
    ...
    fn precedence_tables(&mut self) -> Result<(), String> {
        ...
        match self.strings("ext.op.if_else").len() {
            0 | 2 => {}
            _ => return Err("ext.op.if_else needs two words".into()),
        }
        if self.has_any("ext.stmt.class") && (!self.has_any("ext.op.member") || (!self.has_any("ext.stmt.class.new") && !self.has_any("ext.stmt.class.unready"))) {
        ...
        let symbol_labels = ["ext.op.index.slice.ellipsis", "ext.op.index.slice", "ext.op.comprehension.async", "ext.op.comprehension.for", "ext.op.comprehension.in", "ext.op.comprehension.if", "ext.syntax.array.spread", "ext.syntax.map.spread", "syntax.group.open", "syntax.group.close", "syntax.call.open", "syntax.call.separator", "syntax.call.close",
            ...
            "ext.op.tuple", "ext.stmt.assert", "ext.stmt.catch.as", "ext.stmt.catch.tuple.open", "ext.stmt.catch.tuple.close", "ext.stmt.catch.group", "ext.stmt.throw.from", "ext.stmt.try", "ext.stmt.catch", "ext.stmt.finally",

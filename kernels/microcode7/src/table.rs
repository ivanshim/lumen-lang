...
ext.lexical.number.point.bare:B ext.lexical.number.separator.after_prefix:B ext.stmt.assign.names.chained:B ext.syntax.call.chained:B ext.op.lambda:L ext.op.lambda.unready:L ext.op.assign.expression:L ext.literal.ellipsis:L ext.literal.ellipsis.unready:L ext.stmt.with:L ext.stmt.with.as:L ext.stmt.with.unready:L ext.op.tuple:L ext.op.tuple.unready:L ext.stmt.class.unready:L ext.op.bit.whole:B ext.op.matrix:L ext.op.matrix.unready:L ext.lexical.line_continuation:L ext.lexical.number.exponent:L ext.op.plus:L ext.stmt.break.levels:B ext.builtin.array:L ext.op.index.append:B ext.stmt.for.collection:B ext.builtin.print_r:L \
...
ext.lexical.string.long:L ext.lexical.string.prefix.raw:L ext.lexical.string.prefix.bytes:L ext.lexical.string.prefix.plain:L ext.lexical.string.prefix.format:L ext.lexical.string.adjacent:B ext.lexical.string.unready:L ext.lexical.string.amiss:L ext.lexical.escape.byte.digits:N ext.lexical.escape.codepoint.digits:N ext.lexical.escape.codepoint.wide:L ext.lexical.escape.codepoint.wide.digits:N ext.lexical.escape.named:L ext.lexical.escape.unavailable:L \
...
impl Table {
    ...
    fn precedence_tables(&mut self) -> Result<(), String> {
        ...
        if self.has_any("ext.stmt.class") {
            let reading_body = self.blocks == Blocks::Indented && self.has_any("ext.stmt.class.unready");
            match reading_body {
                true if !self.has_any("syntax.call.open") || !self.has_any("block.intro") => {
                    return Err("An indented class needs call brackets and block.intro".into());
                }
                false if !self.has_any("ext.op.member") || !self.has_any("ext.stmt.class.new") => {
                    return Err("ext.stmt.class needs ext.op.member and ext.stmt.class.new".into());
                }
                _ => {}
            }
        }
        ...
        let symbol_labels = ["ext.lexical.string.long", "ext.op.index.slice.ellipsis", "ext.op.index.slice", "syntax.group.open", "syntax.group.close", "syntax.call.open", "syntax.call.separator", "syntax.call.close",
            ...
            "ext.op.lambda", "ext.op.assign.expression", "ext.literal.ellipsis", "ext.stmt.with", "ext.stmt.with.as", "ext.stmt.global", "ext.stmt.decorator", "ext.lexical.line_continuation", "ext.op.matrix", "ext.op.tuple", "ext.stmt.const", "ext.stmt.switch", "ext.stmt.case", "ext.stmt.default", "ext.op.plus",

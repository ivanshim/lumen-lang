...
fn scan_code_from(source: &str, table: &Table, first: u32, ended: &mut u32) -> Result<Vec<Token>, String> {
    ...
    while pos < src.len() {
        ...
        *ended = row;
        ...
        if c.is_ascii_digit() || ((table.flag("ext.lexical.number.point.bare") || table.flag("ext.lexical.number.point_edge") || table.flag("ext.lexical.number.point_open"))
            && Some(c) == point && src.get(pos + 1).map_or(false, char::is_ascii_digit))
        {
            ...
            let opened = in_base
                ...
                .find(|(d, l, radix)| {
                    ...
                    k - pos == 1 && src[pos] == *d && at(k) == Some(*l) && (strict || at(first).map_or(false, |x| x.is_digit(*radix)))
                })
                .copied();
            if let Some((_, _, radix)) = opened {
                ...
                if point.is_some() && at(k) == point && (table.flag("ext.lexical.number.point.bare") || table.flag("ext.lexical.number.point_edge") || table.flag("ext.lexical.number.point_open") || at(k + 1).map_or(false, |x| x.is_ascii_digit())) {
                    k += 1;
                    while k < src.len() && (src[k].is_ascii_digit() || apart.contains(&src[k])) {
                        k += 1;
                    }

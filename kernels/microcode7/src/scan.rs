...
impl Quotation<'_> {
    ...
    fn literal(&mut self, body: usize, end: &[char], raw: bool, fields: bool) -> Result<(), String> {
        ...
        if bytes {
            let refused = self.table.single("ext.lexical.string.unready").unwrap_or_default().to_owned();
            self.token(Shape::Unheld, refused);
        } else { self.flush(&mut saved, &mut missing); }

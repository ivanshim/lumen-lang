        ...
        *ended = row;
        ...
        if table.begins_name(c) || prefixed || led {
            ...
            for name in table.prims.keys().chain(table.strings("ext.builtin.print.file.output").iter()).chain(table.strings("ext.builtin.print.file.error").iter()) {
                ...
                if routed && (table.spells("ext.builtin.print.file.output", name) || table.spells("ext.builtin.print.file.error", name) || table.prims.get(name) == Some(&crate::form::Prim::Echo)) { continue; }

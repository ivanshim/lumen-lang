...
impl<'a> Machine<'a> {
    ...
    fn can_be_walked(&mut self, x: &Value) -> Result<(), Escape> {
        if matches!(x, Value::Iterator(_) | Value::Tuple(_) | Value::Set(_) | Value::Vector(_) | Value::Dict(_) | Value::Thing(_) | Value::Progression(_)) {
            return Ok(());
        }
    ...
    fn as_raised(&mut self, told: &str) -> Option<Value> {
        if self.table.single("ext.builtin.core.exhausted") == Some(told) && self.table.has_any("ext.stmt.class.special.stop") {
            ...
            let of = Blueprint { name: self.table.single("ext.stmt.class.special.stop")?.to_owned(), under: None, fields: vec![], methods: vec![], shared: RefCell::new(vec![]), reaches: vec![], answers: vec![], constants: vec![] };
            self.made += 1;
                ...
                Prim::Append | Prim::Replace => {
                    ...
                    if self.table.has_any("ext.stmt.class.special") {
                        ...
                        if let (Some(key), Value::Dict(entries)) = (&key, &target) {
                            ...
                            match original { Value::Shared(cell) | Value::Mutable(cell, _) => { cell.replace(changed); }, _ => self.store(slot, frame, changed)? }
                            return Ok(Value::Nil);
                        }
    ...
    fn value_member(&mut self, receiver: &Value, name: &str, arguments: Vec<Value>, keywords: Vec<(String, Value)>) -> Res<Value> {
        ...
        match outcome {
            ...
            Ok(items) => {
                ...
                else { Err(format!("\0{}", self.table.single("ext.builtin.method.sort.modified").unwrap_or_default()).into()) }
            }
        }
    }
    ...
    fn compare_members(&mut self, operation: Prim, one: &Value, two: &Value) -> Result<Value, String> {
        ...
        Err(["\0", pieces[0].as_str(), word, &pieces[1], &names[0], &pieces[2], &names[1], &pieces[3]].concat())
    }
    ...
    fn prim_values(&mut self, op: Prim, name: &str, v: &[Value]) -> Result<Value, String> {
        ...
        Ok(match op {
            ...
            Prim::HasMember => {
                ...
                Value::Flag((self.table.has_any("ext.builtin.exceptions") && matches!(&v[0], Value::Blueprint(_) | Value::Thing(_))) || matches!(v[0], Value::Member(..)) || own || (self.table.has_any("ext.stmt.class.special") && class.is_some()) || class.map_or(false, |c| c.keeper(&word).is_some() || c.program(&word).is_some() || c.constant(&word).is_some()))
            }
            Prim::Of => {
                n(2)?;
                let called = v[1].bare();
...
impl Machine<'_> {
    ...
    fn core_complaint(&self, key: &str, middle: &str) -> String {
        if key == "core.empty" && self.table.has_any("ext.builtin.method.sort.modified") {
            let words = self.table.strings("ext.builtin.core.empty");
            return format!("\0{}{}{}", words[0], middle, words[1]);
        }
    ...
    fn core_run(&mut self, callable: &Value, values: Vec<Value>) -> Result<Value, String> {
        match callable {
            Value::Thing(_) if self.appointed(callable, 17).is_some() => self.ask_special(callable, 17, &values)?.ok_or_else(|| self.bad_answer()),
            Value::Method(routine, subject) => {
                let mut given = vec![Value::Thing(subject.clone())];
                given.extend(values);
                let bound = Value::Bound(routine.clone(), self.outermost.clone());
                self.core_run(&bound, given)
            }
    ...
    fn core_primitive(&mut self, op: Prim, name: &str, mut input: Vec<Value>, keywords: Vec<(String, Value)>) -> Result<Value, String> {
        ...
        match op {
            ...
            Ordered | Least | Greatest => {
                ...
                let walk = if input.len() > 1 {
                    Self::cursor_value(IteratorKind::Stored(input.clone().into_iter().collect()))
                } else if matches!(input[0], Value::Generator(_)) { input[0].clone() }
                else { self.iterated_value(&input[0])? };
                let Some(mut winner) = self.next_value(&walk)? else { return fallback.ok_or_else(|| self.core_complaint("core.empty", name)); };
                let mut best = if matches!(key, Value::Nil) { winner.clone() } else { self.core_run(&key, vec![winner.clone()])? };
                while let Some(candidate) = self.next_value(&walk)? {

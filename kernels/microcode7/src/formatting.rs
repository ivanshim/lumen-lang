...
impl Layout<'_> {
    ...
    pub fn quote(&self, item: &Value, escaped: bool) -> Answer {
        Ok(match item {
            ...
            // Every other worth stands in a field as it stands wherever
            // else it is written out: by the words the table keeps for
            // it. A field turns down nothing a plain writing shows.
            other => other.render(self.names),
        })
    }

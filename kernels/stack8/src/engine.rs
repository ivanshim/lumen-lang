...
impl<'a> Engine<'a> {
    ...
    pub fn new(lang: &'a Lang, registry: crate::compile::Registry) -> Engine<'a> {
        ...
        let world = idents.iter().map(|n| native_exceptions.get(n).cloned().unwrap_or(Value::Blank)).collect();
        Engine {
            lang,
            native_exceptions,
            world,
    ...
    fn perform(&mut self, op: &Action, argc: usize) -> Flow<()> {
        ...
        let result = match op {
            ...
            Action::HasMember(name) => {
                ...
                Value::Flag(matches!(held, Value::ValueMethod(_)) || (!self.lang.exceptions.is_empty() && matches!(&held, Value::Class(_) | Value::Object(_))) || field || class.map_or(false, |c| c.method(name).is_some() || c.holder(name).is_some() || c.constant(name).is_some()))
            }

        ...
        let result = match b {
            ...
            Builtin::Minimum | Builtin::Maximum | Builtin::Sorted => {
                ...
                for value in values {
                    ...
                    while at > 0 {
                        // Where the language orders its sequences, the
                        // ranking is asked of the language itself, so
                        // that rows and tuples are set against one
                        // another place by place as they are elsewhere.
                        let asked = |first: &Value, second: &Value| -> Res<bool> {
                            Ok(self.dyadic(&Action::Lt, first, second)?.is_true())
                        };
                        let (less, greater) = if self.lang.sequence_values {
                            (asked(&k, &ranked[at-1].0)?, asked(&ranked[at-1].0, &k)?)
                        } else {
                            (match (&k, &ranked[at-1].0) {

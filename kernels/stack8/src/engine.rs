...
impl<'a> Engine<'a> {
    ...
    pub fn new(lang: &'a Lang, registry: crate::compile::Registry) -> Engine<'a> {
        ...
        for (slot, word) in idents.iter().enumerate() {
            if let Some(value) = native_exceptions.get(word) { world[slot] = value.clone(); }
        }
        for (slot, word) in idents.iter().enumerate() {
            if lang.subclass_builtin.contains(word) && !lang.builtins.contains_key(word) {
                world[slot] = Value::Class(Rc::new(Class { name: word.clone(), base: None, answers: vec![], fields: vec![], reaches: vec![], methods: vec![], constants: vec![], shared: RefCell::new(vec![]) }));
            }
        }
        Engine {
            lang,
...
{}", open, given.display(&sp)),
            ...
            match value {
                Value::Tie(pair) => match &pair.0 {
                    ...
                    Value::Flag(false) => match &pair.1.contents() {
                        ...
                        Value::Array(a) | Value::Tuple(a) | Value::Set(a) => items.extend(a.iter().cloned().map(|v| (None, v))),
    ...
    fn run_attempt(&mut self, program: &Rc<Routine>, frame: &mut [Value], instrs: &[Instr], plan: &crate::code::Attempt) -> Flow<Passage> {
        ...
        let ending = match self.run_span(program, frame, instrs, plan.body) {
            Err(Fault::Note(told)) if !self.lang.exceptions.is_empty() => match self.as_fault(&told) {
        ...
        let result = match op {
            ...
            Action::KeyAt | Action::ValueAt => {
                ...
                match &pair[0] {
                    ...
                    Value::Array(items) | Value::Tuple(items) | Value::Set(items) => match items.get(at) {
            ...
            Action::Forge(plan) => {
                ...
                let base = match plan.extends {
                    ...
                    true => match given.next() {
                        Some(v) => Some(self.subclass_base(v.clone()).ok_or_else(|| format!("Class {} cannot stand on {}", plan.name, v.plain()))?),
                ...
                for _ in 0..plan.answers {
                    match given.next() {
                        Some(v) => answers.push(self.subclass_base(v.clone()).ok_or_else(|| format!("Class {} cannot answer to {}", plan.name, v.plain()))?),
                        None => return Err("Stack underflow".to_string().into()),
                    }
                }
            ...
            Action::Make => {
                ...
                if class.builtin_base().is_some() {
                    let result = self.builtin_instance(class, args, false)?;
                    self.data.push(result);
                    return Ok(());
                }
                self.made += 1;
    ...
    fn rem_text(&self, template: &str, arguments: &Value) -> Res<String> {
        ...
        let values: Vec<&Value> = match arguments {
            Value::Array(items) | Value::Tuple(items) | Value::Set(items) => items.iter().collect(),
            one => vec![one],
        };
        let mut used = 0;
        ...
        Ok(match op {
            ...
            Action::Eq | Action::Ne if self.lang.loose_equality => {
                ...
                let alike = match (a, b) {
                    ...
                    (one, other) | (other, one) if nothing(one) && matches!(other, Value::Array(_) | Value::Map(_)) => !self.truth(other) || {
                        match other {
                            Value::Array(items) | Value::Tuple(items) | Value::Set(items) => items.is_empty(),
            ...
            Action::Contains | Action::Lacks => {
                let found = match b {
                    ...
                    Value::Array(items) | Value::Tuple(items) | Value::Set(items) => items.iter().any(|v| Self::member_matches(a, v)),
    ...
    fn keyed_places(v: &Value) -> Vec<(Value, Value)> {
        match v {
            Value::Array(items) | Value::Tuple(items) | Value::Set(items) => items.iter().enumerate().map(|(i, x)| (Value::Small(i as i64), x.clone())).collect(),
    ...
    fn element(&self, target: &Value, at: &Value, how: Reading) -> Res<Value> {
        if target.builtin_value().is_some() { return self.element(&target.contents(), at, how); }
    ...
    fn write_slice(&self, target: Value, parts: &[Value; 3], given: Value) -> Res<Value> {
        ...
        let replacement = match given.contents() {
            Value::Array(values) | Value::Tuple(values) | Value::Set(values) => values.as_ref().clone(),
            Value::Text(text) => text.chars().map(|c| Value::text(&c.to_string())).collect(),
            Value::Map(pairs) => pairs.iter().map(|(key, _)| key.clone()).collect(),
    ...
    fn element_held(&self, target: &Value, at: &Value, how: Reading) -> Res<Value> {
        ...
        match target {
            Value::Array(items) | Value::Tuple(items) | Value::Set(items) => match items.get(i) {
                Some(v) => Ok(v.clone()),
    ...
    fn comprehension_items(&mut self, value: &Value) -> Res<Vec<Value>> {
        ...
        match value {
            ...
            Value::Set(items) | Value::Tuple(items) | Value::Array(items) => Ok(items.as_ref().clone()),
            ...
            for value in &mut args {
                if let Value::Object(o) = value {
                    if o.class.builtin_base().is_some() {
                        if let Some(method) = self.lang.subclass_string.first().and_then(|n| o.class.method(n)).cloned() {
                            self.invoke(&method, vec![value.clone()]).map_err(|f| f.told(&self.wording()))?;
                            *value = self.drop_top()?;
                        }
                    }
                }
    ...
    fn builtin_values(&mut self, builtin: Builtin, name: &str, args: &mut Vec<Value>) -> Res<Value> {
        ...
        if !matches!(builtin, Builtin::SortOf | Builtin::InstanceOf | Builtin::Say | Builtin::List | Builtin::Out | Builtin::Append | Builtin::Replace) { for value in args.iter_mut() { *value = value.contents(); } }
        ...
        Ok(match builtin {
            ...
            Builtin::Lead => {
                ...
                match target {
                    Value::Array(items) | Value::Tuple(items) | Value::Set(items) => kept.extend(items.iter().map(|v| (number(), v.clone()))),
...
fn one_after_another(v: &Value) -> Vec<Value> {
    match v {
        ...
        Value::Array(items) | Value::Tuple(items) | Value::Set(items) => items.as_ref().clone(),
...
fn named_pairs(v: &Value) -> Vec<(Value, Value)> {
    match v {
        ...
        Value::Map(pairs) => pairs.as_ref().clone(),
        Value::Array(items) | Value::Tuple(items) | Value::Set(items) => items
            .iter()
            .enumerate()
...
fn laid_out(v: &Value, indent: usize, sp: &Wording) -> String {
    ...
    let (what, pairs): (String, Vec<(String, &Value)>) = match v {
        Value::Array(items) | Value::Tuple(items) | Value::Set(items) => ("Array".to_string(), items.iter().enumerate().map(|(i, x)| (i.to_string(), x)).collect()),
...
impl Engine<'_> {
    ...
    fn subclass_base(&self, value: Value) -> Option<Rc<Class>> {
        ...
        if !self.lang.subclass_builtin.contains(&word.to_string()) { return None; }
        Some(Rc::new(Class { name: word.to_string(), base: None, answers: Vec::new(),
            fields: Vec::new(), reaches: Vec::new(), methods: Vec::new(), shared: RefCell::new(Vec::new()),
            constants: vec![("\0builtin-base".into(), value)],
        }))
    }

    fn builtin_instance(&mut self, class: Rc<Class>, raw: Vec<Value>, allocating: bool) -> Flow<Value> {
        let native = class.builtin_base().expect("a builtin base");
        if !allocating {
            if let Some(method) = self.lang.subclass_allocate.first().and_then(|n| class.method(n)).cloned() {
                let mut given = vec![Value::Class(class.clone())];
        ...
        self.made += 1;
        let mut fields = class.all_fields();
        fields.push(("\0builtin-value".into(), worth));
        let made = Rc::new(Instance { class: class.clone(), fields: RefCell::new(fields), mark: self.made });
        if !allocating {
            if let Some(method) = self.lang.constructor.as_ref().and_then(|n| class.method(n)).cloned() {
                let mut given = vec![Value::Object(made.clone())];
                given.extend(raw);
                self.invoke(&method, given)?;
                self.drop_top()?;
            }

...
enum Passage {
    ...
    /// The body gave way at a yield; where it stands is kept with the
    /// walk, and nothing beyond this point runs until it steps back in.
    Suspended,
}
...
pub struct Engine<'a> {
    ...
    data: Vec<Value>,
    caught: Vec<Value>,
    /// How many of the held faults belong to whoever asked a suspended
    /// body for its next value. A body's own held faults stand above
    /// them, so that it sees its own first and the caller's behind, and
    /// so that what it puts away while it sleeps is found again at the
    /// same remove however deep the caller stood.
    held_base: usize,
...
impl<'a> Engine<'a> {
    fn exception_classes(names: &[String]) -> HashMap<String, Value> {
        let parents = [None, Some(0), Some(1), Some(2), Some(2), Some(1), Some(5), Some(5), Some(1), Some(1), Some(1), Some(10), Some(1), Some(1), Some(13), Some(1), Some(1), Some(0), Some(0), Some(1), Some(1), Some(13), Some(9), Some(1), Some(1), Some(24), Some(24), Some(24), Some(24), Some(24), Some(24), Some(24), Some(24), Some(24), Some(24), Some(24), Some(1), Some(0), Some(37), Some(0), Some(20), Some(20), Some(19)];
    ...
    fn exception_instance(&mut self, class: Rc<Class>, args: Vec<Value>, cause: Value) -> Value {
        ...
        // The exhaustion carries what a generator returned, which is the
        // first of the arguments it was made with.
        if self.stop_class(&class) {
            if let Some(name) = &self.lang.stop_value_member {
    ...
    fn stop_of(&mut self, held: &Rc<RefCell<Generator>>) -> Fault {
        let words = self.lang.yield_exhausted.first().cloned().unwrap_or_default();
        let returned = match held.try_borrow() { Ok(state) => state.returned.clone(), Err(_) => Value::Null };
        if matches!(returned, Value::Null) { return Fault::Note(words); }
        let named = self.lang.special_stop.first().cloned();
        match named.and_then(|name| self.native_exceptions.get(&name).cloned()) {
            Some(Value::Class(class)) => Fault::Thrown(self.exception_instance(class, vec![returned], Value::Null)),
            _ => Fault::Note(words),
        }
    }
...
{}", open, given.display(&sp)),
    ...
    ///
    /// A suspended body may stand inside the try, so `at` says where the
    ...
    fn run_attempt(&mut self, program: &Rc<Routine>, frame: &mut [Value], instrs: &[Instr], plan: &crate::code::Attempt, at: usize, mut suspended: Option<&mut Generator>) -> Flow<Passage> {
        ...
        // A body that gave way inside this try left word of the part it
        // stood in; the try is taken up there rather than at its head,
        // and with the depths it stood at then rather than those now.
        let taking = match suspended.as_deref_mut() {
            Some(kept) if kept.resume.last().map_or(false, |step| step.at == at) => kept.resume.pop(),
            _ => None,
        };
        let depth = taking.as_ref().map_or_else(|| self.data.len(), |step| step.floor);
        let active = taking.as_ref().map_or_else(|| self.caught.len(), |step| self.held_base + step.held);
        let ending = match taking.map_or(Phase::Body, |step| step.phase) {
            Phase::Body => {
                ...
                let ran = self.run_portion(program, frame, instrs, plan.body, suspended.as_deref_mut());
        ...
        if let Some(cell) = &plan.context {
            ...
            return ending.map(|end| match end {
                ...
                match ending {
                    Ok(Passage::Along(mark)) if mark == plan.body.1 => match plan.otherwise {
                        Some(span) => {
                            let ran = self.run_portion(program, frame, instrs, span, suspended.as_deref_mut());
                            if matches!(ran, Ok(Passage::Suspended)) {
                                return self.gave_way(suspended, at, depth, active, Phase::Else);
                            }
                            ran.map(|end| match end {
                                Passage::Along(mark) if mark == span.1 => Passage::Along(plan.after),
                                other => other,
                            })
                        }
            ...
            Phase::Else => {
                let span = plan.otherwise.expect("the part run when nothing was raised");
                let ran = self.run_portion(program, frame, instrs, span, suspended.as_deref_mut());
                if matches!(ran, Ok(Passage::Suspended)) {
                    return self.gave_way(suspended, at, depth, active, Phase::Else);
                }
            ...
            Phase::Arm(index, raised) => self.attempt_clauses(program, frame, instrs, plan, at, suspended.as_deref_mut(), raised, Some(index), depth, active),
            Phase::Last(left, saved) => {
                let ending = Self::as_flow(left);
                return self.attempt_last(program, frame, instrs, plan, at, suspended, ending, depth, active, Some(saved));
            }
        };
        ...
        if matches!(ending, Ok(Passage::Suspended)) { return ending; }
        self.attempt_last(program, frame, instrs, plan, at, suspended, ending, depth, active, None)
    }

    /// Word of where a body gave way, kept with the walk so that
    /// stepping back in finds the same watch standing.
    fn gave_way(&self, suspended: Option<&mut Generator>, at: usize, floor: usize, held: usize, phase: Phase) -> Flow<Passage> {
        let held = held.saturating_sub(self.held_base);
        if let Some(kept) = suspended { kept.resume.push(Step { at, floor, held, phase }); }
        Ok(Passage::Suspended)
    }

    /// What a try had come to, put away so that a body giving way in the
    /// last part may carry it, and taken out again on the way back in.
    fn as_left(ending: Flow<Passage>) -> Ending {
        match ending {
            Ok(Passage::Along(mark)) => Ending::Along(mark),
            Ok(Passage::Leaves { to, cycle }) => Ending::Leaves(to, cycle),
            Ok(Passage::Suspended) => Ending::Finished,
            Err(Fault::Thrown(value)) => Ending::Thrown(value),
            Err(Fault::Note(words)) => Ending::Note(words),
            Err(Fault::Stopped(words)) => Ending::Stopped(words),
            Err(Fault::Finished) => Ending::Finished,
        }
    }

    fn as_flow(left: Ending) -> Flow<Passage> {
        match left {
            Ending::Along(mark) => Ok(Passage::Along(mark)),
            Ending::Leaves(to, cycle) => Ok(Passage::Leaves { to, cycle }),
            Ending::Thrown(value) => Err(Fault::Thrown(value)),
            Ending::Note(words) => Err(Fault::Note(words)),
            Ending::Stopped(words) => Err(Fault::Stopped(words)),
            Ending::Finished => Err(Fault::Finished),
        }
    }

    /// The clauses are shown what the body raised until one takes it;
    /// what none takes is raised again. A clause whose body gave way is
    /// taken up at that body, its class already weighed.
    fn attempt_clauses(&mut self, program: &Rc<Routine>, frame: &mut [Value], instrs: &[Instr], plan: &crate::code::Attempt, at: usize, suspended: Option<&mut Generator>, raised: Value, resumed: Option<usize>, depth: usize, active: usize) -> Flow<Passage> {
        let handled = self.attempt_taking(program, frame, instrs, plan, at, suspended, raised, resumed, depth, active);
        // What the walk is still holding stays held while it sleeps.
        if !matches!(handled, Ok(Passage::Suspended)) { self.caught.truncate(active); }
        handled
    }

    fn attempt_taking(&mut self, program: &Rc<Routine>, frame: &mut [Value], instrs: &[Instr], plan: &crate::code::Attempt, at: usize, mut suspended: Option<&mut Generator>, raised: Value, resumed: Option<usize>, depth: usize, active: usize) -> Flow<Passage> {
        if let Some(index) = resumed {
            let arm = &plan.clauses[index];
        ...
        for (index, arm) in plan.clauses.iter().enumerate() {
            ...
            return self.attempt_arm(program, frame, instrs, plan, at, suspended.as_deref_mut(), arm, index, raised, depth, active);
        }
        Err(Fault::Thrown(raised))
    }

    fn attempt_arm(&mut self, program: &Rc<Routine>, frame: &mut [Value], instrs: &[Instr], plan: &crate::code::Attempt, at: usize, mut suspended: Option<&mut Generator>, arm: &crate::code::Taking, index: usize, raised: Value, depth: usize, active: usize) -> Flow<Passage> {
        let outcome = self.run_portion(program, frame, instrs, arm.body, suspended.as_deref_mut());
        if matches!(outcome, Ok(Passage::Suspended)) {
            return self.gave_way(suspended, at, depth, active, Phase::Arm(index, raised));
        }
    ...
    /// The last part, which runs whichever way the body left, and which
    /// may itself give way; what the try had come to waits with it.
    fn attempt_last(&mut self, program: &Rc<Routine>, frame: &mut [Value], instrs: &[Instr], plan: &crate::code::Attempt, at: usize, mut suspended: Option<&mut Generator>, mut ending: Flow<Passage>, depth: usize, active: usize, taken_up: Option<usize>) -> Flow<Passage> {
        let Some(last) = plan.last else { return ending };
        ...
        let finished = self.run_portion(program, frame, instrs, last, suspended.as_deref_mut());
        if matches!(finished, Ok(Passage::Suspended)) {
            return self.gave_way(suspended, at, depth, active, Phase::Last(Self::as_left(ending), saved));
        }
        let finished = self.raised_of_note(finished);
        self.caught.truncate(active);
        match finished {
            Ok(Passage::Along(mark)) if mark == last.1 => self.data.truncate(saved),
    ...
    /// The class raised at a suspension when the walk is ended.
    fn exit_value(&mut self) -> Option<Value> {
        let name = self.lang.yield_exit.first().cloned()?;
        let Some(Value::Class(class)) = self.native_exceptions.get(&name).cloned() else { return None };
        Some(self.exception_instance(class, Vec::new(), Value::Null))
    }
    ...
    fn close_generator(&mut self, held: &Rc<RefCell<Generator>>) -> Flow<Value> {
        let sleeping = {
            let state = held.try_borrow().map_err(|_| self.lang.yield_busy[0].clone())?;
            state.started && !state.closed && state.program.is_some()
        };
        let exit = self.exit_value().filter(|_| sleeping);
        ...
        match self.step_generator(held, Value::Null, Some(exit), &[]) {
            ...
            Err(other) => { self.shut_generator(held)?; Err(other) }
        }
    }
    ...
    fn step_generator(&mut self, held: &Rc<RefCell<Generator>>, sent: Value, mut hurled: Option<Value>, given: &[Value]) -> Flow<Option<Value>> {
        ...
        // caller's, so that it sees its own first and the caller's behind
        // them, as a body called from a clause does.
        ...
        kept.held = self.caught.split_off(self.held_base);
        self.held_base = outer_base;
        kept.resuming = false;
        kept.hurled = None;
    ...
    fn run_portion(&mut self, program: &Rc<Routine>, frame: &mut [Value], instrs: &[Instr], span: (usize, usize), mut suspended: Option<&mut Generator>) -> Flow<Passage> {
        ...
        // there, so that the tries around it are shown it.
        let mut pc = span.0;
        let mut hurled = None;
        if let Some(kept) = suspended.as_deref_mut() {
            if kept.resuming {
                match kept.resume.last() {
                    Some(step) => pc = step.at,
                    None => { pc = kept.pc; kept.resuming = false; hurled = kept.hurled.take(); }
                }
            }
        }
        if let Some(value) = hurled {
        ...
        while pc >= span.0 && pc < span.1 {
            ...
            match &instrs[pc] {
                ...
                Instr::Attempt(plan) => {
                    // A gathered clause cannot yet keep the pieces of a
                    // group it has parted while a body sleeps.
                    if suspended.is_some() && plan.clauses.iter().any(|arm| arm.grouped)
                        && instrs[plan.body.0..plan.after].iter().any(|i| matches!(i, Instr::Act(Action::Suspend | Action::Delegate, _))) {
                        return Err(self.lang.yield_unsupported[0].clone().into());
                    }
                    match self.run_attempt(program, frame, instrs, plan, pc, suspended.as_deref_mut())? {
                        Passage::Suspended => return Ok(Passage::Suspended),
    ...
    /// The words for a member something has not got: what the thing
    /// sought upon is called stands between the first two and the
    /// member's name between the last two. Nothing at all where the
    /// language words no such complaint.
    fn member_said(words: &Option<(String, String, String)>, called: &str, name: &str) -> String {
        match words {
            Some((before, between, after)) => format!("{before}{called}{between}{name}{after}"),
            None => String::new(),
        }
    }
    ...
    /// builtin kind the word that spells it. Nothing where the value
    ...
    fn kind_word_of(&self, value: &Value) -> Option<String> {
        match value {
            ...
            other => self.kind_spelled(other).map(|word| word.to_string()),
        }
    }

    /// The complaint for a member sought on a module or on a kind, both
    /// of which CPython words by their own name rather than by the kind
    /// they are of. Nothing for anything else, and nothing where the
    /// language words no such complaint.
    ...
    /// The complaint for a member read on a value of a builtin kind
    /// that answers to no such name.
    pub(super) fn member_amiss(&self, value: &Value, name: &str) -> String {
        let told = self.member_named_amiss(value, name);
        if !told.is_empty() { return told; }
        Self::member_said(&self.lang.member_amiss, &value.contents().core_kind(), name)
    }

    /// The complaint for a member written on such a value, or taken
    /// away from it: it keeps no namespace of its own to hold one.
    pub(super) fn member_unwritable(&self, value: &Value, name: &str) -> String {
        let words = if self.lang.member_unwritable.is_some() { &self.lang.member_unwritable } else { &self.lang.member_amiss };
        Self::member_said(words, &value.contents().core_kind(), name)
    }

    /// How the definition spells a value's method, so that a complaint
    /// names the member the way a program writes it.
    fn spelled_member(&self, operation: &str) -> String {
        if self.lang.value_methods.get(operation).map_or(false, |op| op == operation) { return operation.to_string(); }
        let mut words: Vec<&String> = self.lang.value_methods.iter().filter(|(_, op)| op.as_str() == operation).map(|(word, _)| word).collect();
        words.sort();
        words.first().map_or_else(|| operation.to_string(), |word| (*word).clone())
    }

    /// A value's method bound to the value, as reading the member of it
    ...
    pub(super) fn bound_value_method(&mut self, target: Value, operation: &str) -> Res<Value> {
        ...
        Ok(Value::ValueMethod(Rc::new((target, operation.to_string()))))
    }

    /// The member a value of a builtin kind answers to by name: the
    /// method bound to it, or the part it keeps under that name.
    ...
    pub(super) fn builtin_member(&mut self, value: &Value, name: &str) -> Res<Option<Value>> {
        ...
        if let Value::Complex(z) = &held {
            ...
            if Lang::spells(&self.lang.complex_words["ext.builtin.complex.imag"], name) { return Ok(Some(crate::complex::real(z.imag))); }
        }
        if let Value::Counted(row) = &held {
            if let Some(at) = self.range_member_named(name) {
    ...
    fn native_family(subject: &Value) -> Option<Kindred> {
        ...
        Some(match held {
            ...
            Value::View(view) => Kindred::View(view.1.as_str() == "values"),
            _ => return None,
        })
    }

    /// Whether a value of that family answers the special name standing
    /// at that place in the language's list. A walk hands over its next
    /// member and itself; anything walked over hands over a walk of it;
    /// the containers answer for their length, for a place read of them
    /// and for membership; a row, a map and a changeable run of bytes
    /// answer for a place written into, and a row and a map for one
    ...
    /// Whether a working of the first family will take a value of the
    /// second, or turn it down so that the other side may answer. A
    /// whole number works with whole numbers alone, a real with those
    /// and reals, and a container with its own kind.
    fn family_takes(family: Kindred, other: Option<Kindred>) -> bool {
        let Some(other) = other else { return false };
        match family {
            Kindred::Whole => other == Kindred::Whole,
            Kindred::Real => matches!(other, Kindred::Whole | Kindred::Real),
    ...
    /// Whether the member at that place turns a value it cannot take
    /// down rather than refusing it. The comparisons and the workings a
    /// number answers with do; the joining and the repeating a row
    /// answers with refuse outright, as the plain signs do.
    fn family_turns_down(family: Kindred, place: usize) -> bool {
        let joined = matches!(family, Kindred::Text | Kindred::Bytes(_) | Kindred::Row | Kindred::Tuple);
        if joined && matches!(place, 18 | 20 | 28 | 23 | 31) { return false; }
        matches!(place, 2..=7 | 18..=24 | 26..=32 | 60..=71)
    }
    ...
    /// many cells lie between the name and the container itself.
    fn holding_cell(subject: &Value) -> Option<Rc<RefCell<Value>>> {
        let (Value::Collection(cell, _) | Value::Bond(cell) | Value::Binding(cell)) = subject else { return None };
    ...
    /// A special member of a builtin value, run by the place its name
    /// stands at. Nothing is worked out afresh here: the reading, the
    ...
    fn native_member_call(&mut self, receiver: &Value, operation: &str, place: usize, args: Vec<Value>, named: Vec<(String, Value)>) -> Res<Value> {
        ...
        if wanted == 1 && Self::family_turns_down(family, place) && !Self::family_takes(family, Self::native_family(&args[0])) {
            let word = self.lang.special_declined.first().cloned().unwrap_or_default();
            return Ok(Value::Declined(Rc::from(word.as_str())));
        }
        // The members a builtin of one value answers for.
        ...
        if matches!(place, 60 | 61) {
            let pair = if place == 60 { [receiver.clone(), args[0].clone()] } else { [args[0].clone(), receiver.clone()] };
            return self.builtin_call(Builtin::Divmod, operation, pair.into_iter().map(|value| (None, value)).collect());
        }
        // A place written into and a place taken out. The container's
        // own cell takes the change, so that every name for it sees it,
        // and neither writing is worth anything, as the definition has
        // it. What the cell is handed is the container itself and never
        // another cell holding it: a cell within a cell is still the
        // same container to a reading, but a working that writes where
        // the container lies looks for the container and finds a cell.
        if place == 12 {
            let cell = Self::holding_cell(receiver).unwrap_or_else(|| Rc::new(RefCell::new(receiver.contents())));
            let held = cell.borrow().clone();
            let written = self.builtin_call(Builtin::Replace, operation, vec![(None, args[0].clone()), (None, args[1].clone()), (None, held)])?;
            *cell.borrow_mut() = written.contents();
            return Ok(Value::Null);
        }
        if place == 13 {
            // A container with no cell of its own is written into a cell
            // made here and then let go, as the definition has a place
            // taken out of a row no name holds.
            let cell = Self::holding_cell(receiver).unwrap_or_else(|| Rc::new(RefCell::new(receiver.clone())));
            let depth = self.data.len();
            self.data.push(Value::Bond(cell));
            self.data.push(args[0].clone());
            return match self.perform(&Action::ForgetWithin, 2) {
                Ok(()) => { self.drop_top().map_err(|_| self.special_fault())?; Ok(Value::Null) }
                Err(Fault::Note(words)) => { self.data.truncate(depth); Err(words) }
                Err(fled) => { self.data.truncate(depth); self.carried = Some(fled); Err(String::new()) }
            };
        }
        // The compound writings: a row grows or repeats where it stands,
        // and a set or a map takes in what the sign gathers. Each hands
        // back the very value it was asked of.
        ...
        if let Some(which) = match place { 58 => Some(0), 57 => Some(1), 48 => Some(2), 59 => Some(3), _ => None } {
            self.special_dyad(&Action::SetWrite(which), receiver, &args[0])?;
            return Ok(receiver.clone());
        }
        // The signs a number wears alone.
        if let Some(sign) = match place { 25 => Some(Action::Negate), 41 => Some(Action::Positive), 44 => Some(Action::BitTurn), _ => None } {
            let depth = self.data.len();
            self.data.push(receiver.clone());
            return match self.perform(&sign, 1) {
        ...
        let (sign, turned) = match place {
            2 => (Action::Eq, false), 3 => (Action::Ne, false),
            4 => (Action::Lt, false), 5 => (Action::Le, false),
            6 => (Action::Gt, false), 7 => (Action::Ge, false),
            11 => (Action::At, false),
            18 => (Action::Add, false), 26 => (Action::Add, true),
            19 => (Action::Sub, false), 27 => (Action::Sub, true),
            20 => (Action::Mul, false), 28 => (Action::Mul, true),
            21 => (Action::DivReal, false), 29 => (Action::DivReal, true),
            22 => (Action::IntDiv, false), 30 => (Action::IntDiv, true),
            23 => (Action::Mod, false), 31 => (Action::Mod, true),
            24 => (Action::Power, false), 32 => (Action::Power, true),
            62 => (Action::BitUp, false), 67 => (Action::BitUp, true),
            63 => (Action::BitDown, false), 68 => (Action::BitDown, true),
            64 => (Action::BitBoth, false), 69 => (Action::BitBoth, true),
            65 => (Action::BitEither, false), 70 => (Action::BitEither, true),
    ...
    fn order_family(value: &Value) -> String {
        ...
        match kind.as_str() { "bytearray" => "bytes".to_string(), "frozenset" => "set".to_string(), _ => kind }
    }
    ...
    fn perform(&mut self, op: &Action, argc: usize) -> Flow<()> {
        ...
        let result = match op {
            ...
            // A member call the receiver's kind does not answer to: a
            ...
            Action::Grab(name) => match { let top = self.drop_top()?; if let (Value::Bond(cell), false) = (&top, self.native_special(&top, name)) { cell.borrow().clone() } else { top } } {
                ...
                Value::Object(o) => {
                    ...
                    match found {
                        ...
                        None => {
                            ...
                            // A module names itself and the member it
                ...
                // A value of a builtin kind hands over the member its
                // kind gives it, and otherwise names its kind and the
                // member it has not got, where the language words that.
                v => match self.builtin_member(&v, name)? {
                    Some(member) => member,
                    None => {
                        self.absent_member = Some((name.to_string(), v.clone()));
                        let told = self.member_amiss(&v, name);
                        if !told.is_empty() { return Err(told.into()); }
            ...
            Action::Send(name) => {
                ...
                if let Value::Generator(held) = subject {
                    let result = if Lang::spells(&self.lang.yield_close, name) && args.is_empty() {
                        ...
                        match self.resume_generator(&held, args.remove(0))? {
                            Some(item) => item,
    ...
    fn core_step(&mut self, walk: &Value) -> Res<Option<Value>> {
        if let Value::Generator(state) = walk {
            ...
            return match self.resume_generator(state, Value::Null) {
                ...
                Err(other) => Err(other.told(&self.wording())),
            };
        }
        ...
        let result = match b {
            ...
            Builtin::Next => {
                ...
                match self.core_step(&args[0])?.or_else(|| args.get(1).cloned()) {
                    ...
                    None => {
                        ...
                        if let Value::Generator(state) = &args[0] {
                            let state = state.clone();
                            if let fault @ Fault::Thrown(_) = self.stop_of(&state) { self.carried = Some(fault); }
                        }
            ...
            Builtin::HasAttr | Builtin::GetAttr | Builtin::SetAttr | Builtin::DelAttr | Builtin::Vars => {
                ...
                let Value::Object(o) = &args[0] else {
                    ...
                    // kind gives it and keeps no namespace of its own,

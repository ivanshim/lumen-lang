...
pub struct Lang {
    ...
    pub fmt_text_format_digits: Vec<String>,
    pub fmt_text_format_width_big: Vec<String>,
    pub fmt_text_format_precision_big: Vec<String>,
    ...
    pub fmt_text_format_brace_single: Vec<String>,
    ...
    pub fmt_op_rem_format_byte: Vec<String>,
    ...
    /// Whether arithmetic is held to the kinds it has a meaning
    /// for, refusing text and nothing where a number is wanted
    /// rather than reading them for the numbers they might spell.
    ...
    /// The words for a row written into while it is being put in order.
    pub sort_modified: Vec<String>,
    ...
    pub prefix_incompatible: Option<(String, String, String)>,
    pub string_amiss: Option<String>,
    pub string_unterminated: Option<String>,
    pub string_unterminated_triple: Option<String>,
    ...
    pub to_string_undecodable: Vec<String>,
    pub to_string_unready: Vec<String>,
    pub range_zero: Vec<String>,
    pub range_integer: Vec<String>,
    pub range_index: Vec<String>,
    /// What a counted row calls its first place, its stopping place and
    /// its stride, read as members of the row itself.
    pub range_members: Vec<String>,
    /// The words for a worth that stands nowhere in a counted row,
    /// given before and after the worth that was looked for.
    ...
    /// The member on which an exhaustion carries what a generator
    /// returned.
    pub stop_value_member: Option<String>,
    ...
    /// The word a walk's own "is it running" flag answers to, as
    /// `gi_running` on a generator: absent where a language has none.
    pub yield_running: Vec<String>,
    pub yield_unstarted: Vec<String>,
    pub yield_busy: Vec<String>,
    pub yield_unsupported: Vec<String>,
    /// The class raised into a generator being shut, and the complaint
    /// for a generator that takes it and hands out another value.
    /// Two pieces around the kind of a thing thrown into a walk that is
    /// no exception at all.
    pub yield_throw_invalid: Vec<String>,
    pub yield_exit: Vec<String>,
    pub yield_close_ignored: Vec<String>,
    ...
    /// The three pieces that set the file and the line a reading stopped
    /// on about the complaint (ext.builtin.source.syntax.place).
    pub source_place: Vec<String>,
    ...
    pub membership_text: Option<String>,
    pub membership_uncontained: Vec<String>,
    pub membership_declined: Vec<String>,
    ...
    /// The words for a place filled twice over, once in order and once
    /// by name. They name the routine before the place, which the words
    /// for a keyword handed twice do as well.
    pub call_place_twice: Vec<String>,
    /// The words for a call given the same keyword twice: at the run,
    /// where a spread of pairs brought the second (ext.syntax.call.amiss.keyword),
    /// and while reading, where both were written out (ext.syntax.call.amiss.repeated).
    pub call_keyword_twice: Vec<String>,
    pub call_keyword_repeated: Vec<String>,
    pub call_builtin_amiss: Vec<String>,
    /// The words for places a call left empty, for more positional
    /// arguments than there are places, for a keyword no place answers
    /// to, and for places taken only in order that were named
    /// (ext.syntax.call.amiss.absent, .absent.names, .excess,
    /// .unexpected and .ordered). Each names the routine; left unsaid,
    /// the plainer words above are said instead.
    pub call_absent: Vec<String>,
    pub call_absent_names: Vec<String>,
    pub call_excess: Vec<String>,
    pub call_unexpected: Vec<String>,
    pub call_ordered: Vec<String>,
    /// The words for an argument written in order after a named one or
    /// after a spread of pairs (ext.syntax.call.amiss.order).
    pub call_order: Vec<String>,
    pub spread_amiss: Vec<String>,
    pub spread_pairs_amiss: Vec<String>,
    pub parameters_amiss: Vec<String>,
    /// The words for a parameter list naming the same parameter twice
    /// (ext.stmt.function.parameters.duplicate).
    pub parameters_duplicate: Vec<String>,
    ...
    /// The name a routine written with the `lambda` keyword (or its
    /// like) answers to on its own, in place of the word every other
    /// anonymous routine is compiled under. Absent, it keeps that word.
    pub lambda_name: Vec<String>,
...
const EXT_LABELS: &str = "w ext.stmt.class.detail.call | w ext.stmt.class.detail.root | w ext.stmt.class.detail.mro | w ext.stmt.class.detail.order | w ext.stmt.class.detail.name | w ext.stmt.class.detail.qualified | w ext.stmt.class.detail.bases | w ext.stmt.class.detail.namespace | w ext.stmt.class.detail.kind | w ext.stmt.class.detail.allocate | w ext.stmt.class.detail.subclass | w ext.stmt.class.detail.slots | w ext.stmt.class.detail.set | w ext.stmt.class.detail.remove | w ext.stmt.class.detail.get | w ext.stmt.class.detail.getitem | w ext.stmt.class.detail.doc | w ext.stmt.class.detail.module | w ext.stmt.class.detail.defaults | w ext.stmt.class.detail.code | w ext.stmt.class.detail.argcount | w ext.stmt.class.detail.varnames | w ext.stmt.class.detail.receiver | w ext.stmt.class.detail.function | w ext.stmt.class.detail.locals | w ext.stmt.class.detail.main | w ext.stmt.class.detail.mro.amiss | w ext.stmt.class.detail.attribute.amiss | w ext.stmt.class.detail.attribute.readonly | w ext.stmt.class.detail.unready | w ext.stmt.class.detail.descriptor.get | w ext.stmt.class.detail.descriptor.set | w ext.stmt.class.detail.descriptor.delete | w ext.stmt.class.detail.descriptor.name | w ext.stmt.class.detail.descriptor.foreign | w ext.stmt.class.detail.descriptor.unbound | w ext.stmt.class.detail.property.fget | w ext.stmt.class.detail.property.fset | w ext.stmt.class.detail.property.fdel | w ext.stmt.class.detail.property.getter | w ext.stmt.class.detail.property.deleter | w ext.stmt.class.detail.property.doc | w ext.stmt.class.detail.property.readonly | w ext.stmt.class.detail.property.unreadable | w ext.stmt.class.detail.property.unwritable | w ext.stmt.class.detail.property.undeletable | w ext.stmt.class.detail.namespace.amiss | w ext.stmt.class.detail.namespace.kept | w ext.stmt.class.detail.text.amiss | w ext.stmt.class.detail.globals | w ext.stmt.class.detail.closure | w ext.stmt.class.detail.keywords | w ext.stmt.class.detail.defaults.amiss | w ext.stmt.class.detail.keywords.amiss | w ext.stmt.class.detail.code.amiss | w ext.stmt.class.detail.code.free | w ext.stmt.class.detail.cell.contents | w ext.stmt.class.detail.cell.empty | w ext.stmt.class.detail.method.fixed | w ext.stmt.class.detail.kind.fixed | w ext.stmt.class.detail.kind.kept | w ext.stmt.class.detail.arguments.none | w ext.stmt.class.detail.arguments.init | w ext.stmt.class.detail.arguments.new | w ext.stmt.class.detail.root.members | w ext.builtin.issubclass | w ext.builtin.callable | w ext.builtin.getattr | w ext.builtin.setattr | w ext.builtin.delattr | w ext.builtin.hasattr | w ext.builtin.vars | w ext.builtin.dir | w ext.builtin.staticmethod | w ext.builtin.classmethod | w ext.builtin.property | w ext.builtin.text.fault.protocol | b ext.op.index.text.negative | w ext.builtin.text.fault.index | w ext.builtin.text.splitlines | w ext.builtin.text.partition | w ext.builtin.text.rpartition | w ext.builtin.text.expandtabs | w ext.builtin.text.swapcase | w ext.builtin.text.casefold | w ext.builtin.text.capitalize | w ext.builtin.text.title | w ext.builtin.text.istitle | w ext.builtin.text.isidentifier | w ext.builtin.text.isprintable | w ext.builtin.text.isdecimal | w ext.builtin.text.isnumeric | w ext.builtin.text.isascii | w ext.builtin.text.removeprefix | w ext.builtin.text.removesuffix | w ext.builtin.text.format_map | w ext.builtin.text.maketrans | w ext.builtin.text.translate | w ext.builtin.text.encode | w ext.builtin.text.join | w ext.builtin.text.split | w ext.builtin.text.rsplit | w ext.builtin.text.strip | w ext.builtin.text.lstrip | w ext.builtin.text.rstrip | w ext.builtin.text.center | w ext.builtin.text.ljust | w ext.builtin.text.rjust | w ext.builtin.text.zfill | w ext.builtin.text.count | w ext.builtin.text.find | w ext.builtin.text.rfind | w ext.builtin.text.index | w ext.builtin.text.rindex | w ext.builtin.text.startswith | w ext.builtin.text.endswith | w ext.builtin.text.replace | w ext.builtin.text.upper | w ext.builtin.text.lower | w ext.builtin.text.length | w ext.builtin.text.repr | w ext.builtin.text.keyword.count | w ext.builtin.text.keyword.keepends | w ext.builtin.text.keyword.tabsize | w ext.builtin.text.keyword.maxsplit | w ext.builtin.text.keyword.sep | w ext.builtin.text.keyword.encoding | w ext.builtin.text.keyword.errors | w ext.builtin.text.fault.arguments | w ext.builtin.text.fault.arguments.many | w ext.builtin.text.fault.receiver | w ext.builtin.text.fault.string | w ext.builtin.text.fault.integer | w ext.builtin.text.fault.separator | w ext.builtin.text.fault.fill | w ext.builtin.text.fault.missing | w ext.builtin.text.fault.encode | w ext.builtin.text.fault.mapping | w ext.builtin.text.fault.translation | w ext.builtin.text.fault.codepoint | w ext.builtin.text.fault.surrogate | w ext.builtin.text.fault.maketrans.length | w ext.builtin.text.fault.maketrans.key | w ext.builtin.text.fault.maketrans.type | w ext.builtin.text.fault.format | w ext.builtin.text.fault.format.positional | w ext.builtin.text.fault.format.brace | w ext.builtin.text.fault.join | w ext.builtin.text.fault.walk | w ext.builtin.text.fault.room | w ext.builtin.text.fault.key | w ext.builtin.text.complaint | b ext.builtin.text.repeat | w ext.builtin.bytes.signed | w ext.system.bytes.strict | w ext.builtin.bytes | w ext.builtin.bytearray | w ext.builtin.bytes.encode | w ext.builtin.bytes.decode | w ext.builtin.bytes.hex | w ext.builtin.bytes.fromhex | w ext.builtin.bytearray.fromhex | w ext.builtin.bytes.maketrans | w ext.builtin.bytes.upper | w ext.builtin.bytes.lower | w ext.builtin.bytes.split | w ext.builtin.bytes.join | w ext.builtin.bytes.startswith | w ext.builtin.bytes.replace | w ext.builtin.bytes.strip | w ext.builtin.bytes.find | w ext.builtin.bytes.from_int | w ext.builtin.bytes.to_int | w ext.system.bytes.repr | w ext.system.bytes.type | w ext.system.bytes.encodings | w ext.system.bytes.order | w ext.system.bytes.unready | w ext.system.bytes.arguments | w ext.system.bytes.range | w ext.system.bytes.negative | w ext.system.bytes.index | w ext.system.bytes.subscript | w ext.system.bytes.concat | w ext.system.bytes.missing | w ext.system.bytes.immutable | w ext.system.bytes.unhashable | w ext.system.bytes.separator | w ext.system.bytes.hex | w ext.system.bytes.overflow | w ext.system.bytes.unsigned | w ext.system.bytes.bad_order | w ext.system.bytes.decode | w ext.system.bytes.encode | w ext.lexical.string.bytes.ascii | w ext.lexical.string.bytes.mixed | w ext.text.format.zero.integer | w ext.text.format.zero.string | w ext.op.rem.format.nan | w ext.op.rem.format.infinity | b ext.lexical.number.point.open | w ext.builtin.format | w ext.text.format | w ext.text.format.invalid | w ext.text.format.unknown | w ext.text.format.kinds | w ext.text.format.unready | w ext.text.format.digits | w ext.text.format.width.big | w ext.text.format.precision.big | w ext.text.format.precision.integer | w ext.text.format.precision.missing | w ext.text.format.sign.string | w ext.text.format.alternate.string | w ext.text.format.align.string | w ext.text.format.sign.character | w ext.text.format.alternate.character | w ext.text.format.character | w ext.text.format.spec.type | w ext.text.format.numbered.auto | w ext.text.format.numbered.manual | w ext.text.format.index | w ext.text.format.key | w ext.text.format.brace.open | w ext.text.format.brace.close | w ext.text.format.brace.single | w ext.text.format.conversion | w ext.text.format.recursion | w ext.op.rem.format.few | w ext.op.rem.format.many | w ext.op.rem.format.mapping | w ext.op.rem.format.number | w ext.op.rem.format.integer | w ext.op.rem.format.real | w ext.op.rem.format.character | w ext.op.rem.format.star | w ext.op.rem.format.incomplete | w ext.op.rem.format.code | w ext.lexical.line_continuation | b ext.lexical.number.point.bare | b ext.lexical.number.separator.after_prefix | b ext.op.bit.whole | b ext.builtin.print.real_point | w ext.lexical.string.long | w ext.op.lambda | w ext.op.tuple | w ext.stmt.class.bases.open | w ext.stmt.class.bases.close | w ext.stmt.class.unready | w ext.stmt.class.builtin | w ext.stmt.class.layout | w ext.stmt.class.missing | w ext.stmt.del | w ext.stmt.nonlocal | w ext.stmt.nonlocal.unrun | w ext.stmt.with | w ext.stmt.with.as | w ext.stmt.yield | w ext.stmt.yield.from | w ext.stmt.yield.unrun | w ext.system.scope.unready | w ext.stmt.type_params.close | w ext.stmt.type_params.open | w ext.stmt.for.target.unready | w ext.op.identity.unready | w ext.op.identity.negated | w ext.op.identity | w ext.op.in.unready | w ext.op.in.negated | w ext.op.in | w ext.stmt.del.unrun | w ext.stmt.async.unready | w ext.op.await | w ext.stmt.async | w ext.stmt.with.unready | w ext.op.lambda.unready | b ext.op.member.pipes | w ext.op.tuple.unready | w ext.lexical.number.imaginary.unready | w ext.lexical.number.imaginary | w ext.lexical.string.amiss | w ext.lexical.string.unterminated | w ext.lexical.string.unterminated.triple | w ext.lexical.string.prefix.format.unterminated | w ext.lexical.string.prefix.format.unterminated.triple | w ext.lexical.line_continuation.amiss | w ext.lexical.string.prefix.raw | w ext.lexical.string.prefix.plain | w ext.lexical.string.prefix.bytes | w ext.lexical.string.prefix.format | w ext.lexical.string.prefix.incompatible | b ext.lexical.string.adjacent | w ext.lexical.string.unready | b ext.lexical.escape.continued | b ext.stmt.loop.else | w ext.stmt.with.unrun | w ext.stmt.async.unrun | w ext.stmt.match | w ext.stmt.match.case | w ext.stmt.type_alias | b ext.stmt.type_parameters | b ext.op.pipe.attribute | b ext.stmt.yield.suspends | w ext.stmt.yield.exhausted | w ext.stmt.yield.send | w ext.stmt.yield.close | w ext.stmt.yield.throw | w ext.stmt.yield.running | w ext.stmt.yield.unstarted | w ext.stmt.yield.busy | w ext.stmt.yield.unsupported | w ext.stmt.yield.throw.invalid | w ext.stmt.yield.exit | w ext.stmt.yield.close.ignored | w ext.builtin.next | w ext.builtin.iter | w ext.builtin.tuple | w ext.stmt.class.suite | w ext.stmt.class.suite.unsupported | w ext.lexical.string.raw_prefix | w ext.lexical.string.text_prefix | b ext.syntax.string.adjacent | w ext.op.identity.unsupported | b ext.op.comparison.chain | b ext.stmt.for.range.stop | w ext.stmt.with.group.open | w ext.stmt.with.group.close | w ext.stmt.with.unsupported | w ext.stmt.nonlocal.unsupported | w ext.stmt.delete | w ext.stmt.delete.unsupported | w ext.syntax.tuple.separator | w ext.syntax.tuple.unsupported | w ext.syntax.value.spread | w ext.syntax.value.spread.unsupported | w ext.op.index.spread.unsupported | w ext.op.conditional | b ext.stmt.function.short.bare | b ext.lexical.string.triple | w ext.stmt.class.special.unready | w ext.stmt.class.special.declined | w ext.stmt.class.special.stop | w ext.stmt.class.special | w ext.stmt.class.special.amiss | w ext.builtin.repr | w ext.builtin.hash | w ext.builtin.bool | w ext.builtin.sorted | w ext.builtin.isinstance | b ext.op.arithmetic.binary | b ext.op.arithmetic.flags | b ext.op.arithmetic.strict | w ext.builtin.to_real.infinity | w ext.builtin.to_real.nan | b ext.lexical.number.point_open | b ext.op.pow.real_exponent | w ext.op.pow.overflow | w ext.op.pow.nonreal | w ext.op.pow.zero | w ext.op.div.zero | w ext.op.quot.zero | w ext.op.quot.real_zero | w ext.op.rem.real_zero | w ext.builtin.to_int.text.detail | b ext.op.bit.unbounded | w ext.op.bit.integer | w ext.op.bit.beyond | w ext.stmt.class.index.amiss | w ext.stmt.class.binary.amiss | w ext.stmt.class.format.amiss | w ext.stmt.class.metaclass
...
w ext.op.comprehension.async | w ext.op.comprehension.async.unavailable | w ext.op.comprehension.target.unavailable | w ext.builtin.sum.non_number | w ext.builtin.range.non_integer | w ext.builtin.range.zero_step | w ext.op.comprehension.for | w ext.op.comprehension.in | w ext.op.comprehension.if | b ext.syntax.set | w ext.syntax.array.spread | w ext.syntax.map.spread | w ext.syntax.collection.unwalkable | w ext.syntax.map.spread.unmapped | w ext.op.comprehension.unpack.amiss | b ext.builtin.range.value | w ext.builtin.sum | w ext.builtin.list | w ext.builtin.any | w ext.lexical.epilogue | w ext.system.args.list | w ext.system.args.count | w ext.lexical.prologue.echo | b ext.lexical.prologue.folded | w ext.builtin.echo | b ext.syntax.call.bare | w ext.op.increment | b ext.lexical.number.point.bare | b ext.lexical.number.separator.after_prefix | b ext.stmt.assign.names.chained | b ext.syntax.call.chained | w ext.op.lambda | w ext.op.lambda.unready | w ext.op.assign.expression | w ext.literal.ellipsis | w ext.literal.ellipsis.unready | w ext.stmt.with | w ext.stmt.with.as | w ext.stmt.with.unready | w ext.op.tuple | w ext.op.tuple.unready | w ext.stmt.class.unready | b ext.op.bit.whole | w ext.op.matrix | w ext.op.matrix.unready | w ext.lexical.line_continuation | w ext.op.contains | w ext.op.in.text | w ext.op.in.uncontained | w ext.op.in.declined | b ext.op.bit.or.maps | b ext.op.eq.maps.unordered | w ext.builtin.print.separator | w ext.builtin.print.end | w ext.builtin.print.option.type | w ext.builtin.map | w ext.builtin.map.arguments.amiss | w ext.builtin.map.pair.amiss | w ext.builtin.method.sort.key | w ext.builtin.method.sort.reverse | w ext.builtin.method.sort.modified | w ext.builtin.method.split.sep | w ext.builtin.method.split.maxsplit | w ext.builtin.method.upper | w ext.builtin.method.lower | w ext.builtin.method.strip | w ext.builtin.method.lstrip | w ext.builtin.method.rstrip | w ext.builtin.method.split | w ext.builtin.method.rsplit | w ext.builtin.method.join | w ext.builtin.method.replace | w ext.builtin.method.startswith | w ext.builtin.method.endswith | w ext.builtin.method.find | w ext.builtin.method.rfind | w ext.builtin.method.rindex | w ext.builtin.method.index | w ext.builtin.method.count | w ext.builtin.method.isdigit | w ext.builtin.method.isalpha | w ext.builtin.method.isalnum | w ext.builtin.method.isspace | w ext.builtin.method.islower | w ext.builtin.method.isupper | w ext.builtin.method.title | w ext.builtin.method.capitalize | w ext.builtin.method.swapcase | w ext.builtin.method.center | w ext.builtin.method.ljust | w ext.builtin.method.rjust | w ext.builtin.method.zfill | w ext.builtin.method.format | w ext.builtin.method.encode | w ext.builtin.method.append | w ext.builtin.method.extend | w ext.builtin.method.insert | w ext.builtin.method.pop | w ext.builtin.method.remove | w ext.builtin.method.sort | w ext.builtin.method.reverse | w ext.builtin.method.copy | w ext.builtin.method.clear | w ext.builtin.method.get | w ext.builtin.method.keys | w ext.builtin.method.values | w ext.builtin.method.items | w ext.builtin.method.mapping | w ext.builtin.method.setdefault | w ext.builtin.method.update | w ext.builtin.method.bit_length | w ext.builtin.method.is_integer | w ext.builtin.method.hex | w ext.builtin.method.as_integer_ratio | w ext.builtin.method.error.unready | w ext.builtin.method.error.bytes | w ext.builtin.method.error.arguments | w ext.builtin.method.error.separator | w ext.builtin.method.error.substring | w ext.builtin.method.error.pop | w ext.builtin.method.error.index | w ext.builtin.method.error.remove | w ext.builtin.method.error.list_index | w ext.builtin.method.error.format | w ext.builtin.method.error.spec | w ext.builtin.method.error.unicode | w ext.builtin.method.error.attribute | w ext.builtin.method.error.key | w ext.builtin.method.error.missing | w ext.builtin.method.error.mixed | w ext.builtin.method.error.fill | w ext.builtin.method.error.hex | w ext.builtin.method.error.hex_overflow | w ext.builtin.sorted | w ext.builtin.isinstance | w ext.builtin.tuple | w ext.builtin.set | w ext.builtin.frozenset | w ext.builtin.dict | w ext.builtin.reversed | w ext.builtin.enumerate | w ext.builtin.iterable | w ext.builtin.enumerate.missing | w ext.builtin.enumerate.too_many | w ext.builtin.enumerate.keyword | w ext.builtin.reduce_native | w ext.builtin.rebuild_native | w ext.builtin.zip | w ext.builtin.filter | w ext.builtin.all | w ext.builtin.min | w ext.builtin.max | w ext.builtin.abs | w ext.builtin.round | w ext.builtin.divmod | w ext.builtin.pow | w ext.builtin.hex | w ext.builtin.oct | w ext.builtin.bin | w ext.builtin.repr | w ext.builtin.bool | w ext.builtin.callable | w ext.builtin.id | w ext.builtin.hash | w ext.builtin.iter | w ext.builtin.next | w ext.builtin.hasattr | w ext.builtin.getattr | w ext.builtin.setattr | w ext.builtin.delattr | w ext.builtin.vars | w ext.builtin.key | w ext.builtin.reverse | w ext.builtin.start | w ext.builtin.default | w ext.builtin.round.ndigits | w ext.builtin.round.number | w ext.builtin.pow.base | w ext.builtin.pow.exp | w ext.builtin.pow.mod | w ext.builtin.core.uniterable | w ext.builtin.core.unreversible | w ext.builtin.core.uncallable | w ext.builtin.core.unhashable | w ext.builtin.core.unready | w ext.builtin.core.exhausted | w ext.builtin.core.isinstance.amiss | w ext.builtin.core.issubclass.amiss | w ext.builtin.core.issubclass.subject | w ext.builtin.core.empty | w ext.builtin.core.arity | w ext.builtin.core.attribute | w ext.builtin.core.attribute.name | w ext.builtin.core.vars | w ext.builtin.core.zero | w ext.builtin.core.mod.zero | w ext.builtin.core.inverse | w ext.builtin.core.default.many | w ext.builtin.core.dict.pair | w ext.builtin.core.unindexable | w ext.builtin.core.immutable | w ext.builtin.core.power.zero | w ext.builtin.core.power.overflow | w ext.builtin.core.arity.one | w ext.builtin.core.arity.exact | w ext.builtin.core.integer | w ext.builtin.core.not_iterator | w ext.builtin.core.power.integer | w ext.builtin.core.dict.sequence | w ext.builtin.set.add | w ext.builtin.set.remove | w ext.builtin.set.discard | w ext.builtin.set.pop | w ext.builtin.set.clear | w ext.builtin.set.copy | w ext.builtin.set.update | w ext.builtin.set.union | w ext.builtin.set.intersection | w ext.builtin.set.difference | w ext.builtin.set.symmetric_difference | w ext.builtin.set.issubset | w ext.builtin.set.issuperset | w ext.builtin.set.isdisjoint | w ext.builtin.set.intersection_update | w ext.builtin.set.difference_update | w ext.builtin.set.symmetric_difference_update | w ext.builtin.set.sorted | w ext.builtin.set.method.unavailable | w ext.builtin.set.changed | w ext.builtin.set.unhashable | w ext.builtin.set.missing | w ext.builtin.set.empty | w ext.builtin.set.operands | w ext.builtin.set.arguments | w ext.builtin.set.unsupported | w ext.builtin.set.unsortable
...
w ext.builtin.globals | w ext.builtin.locals | w ext.builtin.exec | w ext.builtin.compile | w ext.builtin.compile.modes | w ext.builtin.compile.parameters | w ext.builtin.compile.kind | w ext.builtin.source.syntax | w ext.builtin.source.syntax.place | w ext.builtin.source.unready | w ext.builtin.import | w ext.system.module.doc | w ext.system.module.builtins | w ext.system.names.module | b ext.op.sequence.values | w ext.op.sequence.concat | w ext.op.sequence.repeat | w ext.op.sequence.index | w ext.op.sequence.delete | w ext.op.sequence.subscript | w ext.op.sequence.missing | w ext.op.sequence.assign  | w ext.builtin.ascii  | w ext.text.format.complex.zero | w ext.text.format.complex.align  | w ext.op.rem.format.byte ";
...
impl Lang {
    ...
    /// method's wrong answer, two values in no order, the least or the
    /// greatest of nothing at all.
    pub fn protocol_named(&self, said: &str) -> bool {
        ...
        self.bool_base.as_deref() == Some(said)
            ...
            || self.core_between("core.empty", said)
            ...
            || self.uneven_zip_named("map.short", said) || self.uneven_zip_named("map.long", said)
    ...
    pub fn parse(text: &str) -> Result<Lang, String> {
        ...
        for (tag, native) in [
            ...
            ("ext.builtin.frozenset", Builtin::Frozen),
            ...
            ("ext.builtin.ascii", Builtin::Ascii),
            ...
            ("ext.builtin.bytearray.fromhex", Builtin::Bytes(49)),
            ("ext.builtin.bytes.maketrans", Builtin::Bytes(40)),
            ...
            for lex in r.strings(tag)? {
                ...
                // it, so that a builtin a definition spells more than
                // one way always answers by the one name.
        ...
        let core_words = ["core.integer", "core.not_iterator", "core.power.integer", "core.dict.sequence", "core.unindexable", "core.immutable", "core.power.zero", "core.power.overflow", "core.arity.one", "core.arity.exact", "key", "reverse", "start", "default", "round.ndigits", "round.number", "pow.base", "pow.exp", "pow.mod", "core.uniterable", "core.uncallable", "core.unhashable", "core.unready", "core.exhausted", "core.isinstance.amiss", "core.issubclass.amiss", "core.issubclass.subject", "core.empty", "core.arity", "core.attribute", "core.attribute.name", "core.vars", "core.zero", "core.mod.zero", "core.inverse", "core.default.many", "core.dict.pair", "core.unsized", "core.dict.changed", "zip.strict", "zip.short", "zip.long", "iterable", "enumerate.missing", "enumerate.too_many", "enumerate.keyword", "core.unreversible", "map.short", "map.long"].into_iter().map(|n| Ok((n.to_string(), r.strings(&format!("ext.builtin.{}", n))?))).collect::<Result<HashMap<_, _>, String>>()?;
        let mut lang = Lang {
            core_words,
            ident: name,
            ...
            strict_arithmetic: r.flag("ext.op.arithmetic.strict")?,
            ...
            byte_words: ["ext.builtin.bytes.signed", "ext.system.bytes.strict", "ext.builtin.bytes", "ext.builtin.bytearray", "ext.builtin.bytes.encode", "ext.builtin.bytes.decode", "ext.builtin.bytes.hex", "ext.builtin.bytes.fromhex", "ext.builtin.bytes.maketrans", "ext.builtin.bytes.upper", "ext.builtin.bytes.lower", "ext.builtin.bytes.split", "ext.builtin.bytes.join", "ext.builtin.bytes.startswith", "ext.builtin.bytes.replace", "ext.builtin.bytes.strip", "ext.builtin.bytes.find", "ext.builtin.bytes.from_int", "ext.builtin.bytes.to_int", "ext.builtin.isinstance", "ext.builtin.hash", "ext.system.bytes.repr", "ext.system.bytes.type", "ext.system.bytes.encodings", "ext.system.bytes.order", "ext.system.bytes.unready", "ext.system.bytes.arguments", "ext.system.bytes.range", "ext.system.bytes.negative", "ext.system.bytes.index", "ext.system.bytes.subscript", "ext.system.bytes.concat", "ext.system.bytes.missing", "ext.system.bytes.immutable", "ext.system.bytes.unhashable", "ext.system.bytes.separator", "ext.system.bytes.hex", "ext.system.bytes.overflow", "ext.system.bytes.unsigned", "ext.system.bytes.bad_order", "ext.system.bytes.decode", "ext.system.bytes.encode", "ext.lexical.string.bytes.ascii", "ext.lexical.string.bytes.mixed"].iter().map(|key| Ok((key.to_string(), r.strings(key)?))).collect::<Result<_, String>>()?,
            ...
            fmt_text_format_digits: r.strings("ext.text.format.digits")?,
            fmt_text_format_width_big: r.strings("ext.text.format.width.big")?,
            fmt_text_format_precision_big: r.strings("ext.text.format.precision.big")?,
            ...
            fmt_text_format_brace_single: r.strings("ext.text.format.brace.single")?,
            ...
            fmt_op_rem_format_byte: r.strings("ext.op.rem.format.byte")?,
            ...
            prefix_incompatible: r.about_two("ext.lexical.string.prefix.incompatible")?,
            string_amiss: r.head("ext.lexical.string.amiss")?,
            string_unterminated: r.head("ext.lexical.string.unterminated")?,
            string_unterminated_triple: r.head("ext.lexical.string.unterminated.triple")?,
            ...
            complex_words: ["ext.builtin.complex", "ext.builtin.complex.real", "ext.builtin.complex.imag", "ext.builtin.method.conjugate", "ext.builtin.complex.invalid", "ext.builtin.complex.integer", "ext.builtin.complex.order", "ext.builtin.complex.floor", "ext.builtin.complex.zero", "ext.builtin.complex.power.zero", "ext.builtin.complex.power.overflow", "ext.builtin.complex.power.modulo", "ext.builtin.complex.integer.overflow", "ext.builtin.complex.unready"].into_iter().map(|label| Ok((label.to_string(), r.strings(label)?))).collect::<Result<_, String>>()?,
            ...
            sort_modified: r.strings("ext.builtin.method.sort.modified")?,
            ...
            yield_running: r.strings("ext.stmt.yield.running")?,
            ...
            yield_throw_invalid: r.strings("ext.stmt.yield.throw.invalid")?,
            yield_exit: r.strings("ext.stmt.yield.exit")?,
            yield_close_ignored: r.strings("ext.stmt.yield.close.ignored")?,
            ...
            class_details: ["call", "locals", "root", "mro", "order", "name", "qualified", "bases", "namespace", "kind", "allocate", "subclass", "slots", "set", "remove", "get", "getitem", "doc", "module", "defaults", "code", "argcount", "varnames", "receiver", "function", "main", "mro.amiss", "attribute.amiss", "attribute.readonly", "unready", "descriptor.get", "descriptor.set", "descriptor.delete", "descriptor.name", "descriptor.foreign", "descriptor.unbound", "property.fget", "property.fset", "property.fdel", "property.getter", "property.deleter", "property.doc", "property.readonly", "property.unreadable", "property.unwritable", "property.undeletable", "namespace.amiss", "namespace.kept", "text.amiss", "globals", "closure", "keywords", "defaults.amiss", "keywords.amiss", "code.amiss", "code.free", "cell.contents", "cell.empty", "method.fixed", "kind.fixed", "kind.kept", "arguments.none", "arguments.init", "arguments.new", "root.members"].into_iter().map(|part| Ok((part.to_string(), r.strings(&format!("ext.stmt.class.detail.{}", part))?))).collect::<Result<_, String>>()?,
            ...
            membership_text: r.head("ext.op.in.text")?,
            membership_uncontained: r.strings("ext.op.in.uncontained")?,
            membership_declined: r.strings("ext.op.in.declined")?,
            ...
            set_words: ["ext.builtin.set.method.unavailable","ext.builtin.set.changed","ext.builtin.set","ext.builtin.frozenset","ext.builtin.set.add","ext.builtin.set.remove","ext.builtin.set.discard","ext.builtin.set.pop","ext.builtin.set.clear","ext.builtin.set.copy","ext.builtin.set.update","ext.builtin.set.union","ext.builtin.set.intersection","ext.builtin.set.difference","ext.builtin.set.symmetric_difference","ext.builtin.set.issubset","ext.builtin.set.issuperset","ext.builtin.set.isdisjoint","ext.builtin.set.intersection_update","ext.builtin.set.difference_update","ext.builtin.set.symmetric_difference_update","ext.builtin.set.sorted","ext.builtin.set.unhashable","ext.builtin.set.missing","ext.builtin.set.empty","ext.builtin.set.operands","ext.builtin.set.arguments","ext.builtin.set.unsupported","ext.builtin.set.unsortable"].into_iter().map(|label| Ok((label.to_string(), r.strings(label)?))).collect::<Result<_, String>>()?,
            ...
            call_place_twice: r.strings("ext.syntax.call.amiss.positional")?,
            call_keyword_twice: r.strings("ext.syntax.call.amiss.keyword")?,
            call_keyword_repeated: r.strings("ext.syntax.call.amiss.repeated")?,
            call_builtin_amiss: r.strings("ext.syntax.call.amiss.builtin")?,
            call_absent: r.strings("ext.syntax.call.amiss.absent")?,
            call_absent_names: r.strings("ext.syntax.call.amiss.absent.names")?,
            call_excess: r.strings("ext.syntax.call.amiss.excess")?,
            call_unexpected: r.strings("ext.syntax.call.amiss.unexpected")?,
            call_ordered: r.strings("ext.syntax.call.amiss.ordered")?,
            call_order: r.strings("ext.syntax.call.amiss.order")?,
            ...
            to_string_undecodable: r.strings("ext.builtin.to_string.undecodable")?,
            ...
            parameters_duplicate: r.strings("ext.stmt.function.parameters.duplicate")?,
            ...
            lambda_name: r.strings("ext.stmt.function.anonymous")?,

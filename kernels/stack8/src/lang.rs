...
w ext.op.index.slice.ellipsis | w ext.builtin.slice | w ext.builtin.slice.start | w ext.builtin.slice.stop | w ext.builtin.slice.step | w ext.builtin.slice.arity | w ext.builtin.slice.length | w ext.builtin.method.indices | w ext.builtin.method.slice_hash | w ext.op.index.get | w ext.op.index.set | w ext.op.index.delete | w ext.op.index.integer | w ext.op.index.slice.amiss | w ext.op.index.slice | w ext.op.index.slice.zero | w ext.op.index.slice.bounds | w ext.op.index.slice.unsupported | w ext.op.index.slice.assign | w ext.op.index.slice.length | w ext.op.index.slice.detached
...
impl Lang {
    pub fn parse(text: &str) -> Result<Lang, String> {
        ...
        for (tag, native) in [
            ...
            ("builtin.extern", Builtin::External), ("ext.builtin.slice", Builtin::MakeSlice), ("builtin.range", Builtin::Span), ("builtin.real", Builtin::MakeReal),
        ...
        let mut lang = Lang {
            ...
            slice_words: ["ext.builtin.slice", "ext.builtin.slice.start", "ext.builtin.slice.stop", "ext.builtin.slice.step", "ext.builtin.slice.arity", "ext.builtin.slice.length", "ext.op.index.get", "ext.op.index.set", "ext.op.index.delete", "ext.op.index.integer", "ext.op.index.slice.amiss"].into_iter().map(|key| Ok((key.to_string(), r.head(key)?.unwrap_or_default()))).collect::<Result<_, String>>()?,

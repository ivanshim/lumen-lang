...
w ext.op.index.slice | w ext.op.index.slice.zero | w ext.op.index.slice.bounds | w ext.op.index.slice.unsupported | w ext.op.index.slice.assign | w ext.op.index.slice.length | w ext.op.index.slice.detached
...
impl Lang {
    ...
    fn order_lexemes(&mut self) -> Result<(), String> {
        ...
        let mut lists: Vec<&Vec<String>> = vec![
            ...
            &self.slice_marks, &self.quote_close, &self.increments, &self.decrements, &self.case_marks, &self.decorator_words,
        ];

...
w ext.lexical.string.long | w ext.lexical.string.prefix.raw | w ext.lexical.string.prefix.bytes | w ext.lexical.string.prefix.plain | w ext.lexical.string.prefix.format | b ext.lexical.string.adjacent | w ext.lexical.string.unready | w ext.lexical.string.amiss | n ext.lexical.escape.byte.digits | n ext.lexical.escape.codepoint.digits | w ext.lexical.escape.codepoint.wide | n ext.lexical.escape.codepoint.wide.digits | w ext.lexical.escape.named | w ext.lexical.escape.unavailable
...
impl Lang {
    pub fn parse(text: &str) -> Result<Lang, String> {
        ...
        let mut lang = Lang {
            ...
            string_unready: r.head("ext.lexical.string.unready")?,
        ...
        if !lang.class_words.is_empty() {
            if lang.blocks == Blocks::Indented && lang.class_unready.is_some() {
                if lang.calling.is_none() || lang.block_intros.is_empty() {
                    return Err("An indented class needs call brackets and block.intro".to_string());
                }
            } else if lang.member_mark.is_none() || lang.new_words.is_empty() {
    ...
    fn order_lexemes(&mut self) -> Result<(), String> {
        ...
        let mut lists: Vec<&Vec<String>> = vec![
            ...
            &self.slice_ellipsis, &self.slice_marks, &self.quote_close, &self.increments, &self.decrements, &self.case_marks, &self.decorator_words, &self.line_continuations, &self.matrix_words, &self.tuple_marks, &self.ellipsis_words, &self.lambda_words, &self.expression_assign, &self.with_words, &self.with_as_words,

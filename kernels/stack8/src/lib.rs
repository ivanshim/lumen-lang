...
fn go(lang: &Lang, source: &str, program_args: &[String], request: &[(String, String, String, bool)]) -> Result<(), String> {
    go_inner(lang, source, program_args, request).map_err(|e| {
        ...
        if strings::already_named(lang, &e) || crate::formatting::names_fault(lang, &e) || lang.slice_named(&e) || (lang.import_missing.len() == 2 && e.starts_with(&lang.import_missing[0]) && e.ends_with(&lang.import_missing[1])) { e }

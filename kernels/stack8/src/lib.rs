...
fn go(lang: &Lang, source: &str, program_args: &[String], request: &[(String, String, String, bool)]) -> Result<(), String> {
    go_inner(lang, source, program_args, request).map_err(|e| {
        let complete = [&lang.call_builtin_amiss, &lang.index_amiss, &lang.class_binary_amiss, &lang.format_amiss]
            .iter().any(|words| words.len() >= 2 && e.starts_with(&words[0]) && e.ends_with(words.last().unwrap()));
        if complete { e }
